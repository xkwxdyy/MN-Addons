  ...
  'SimplePanelController : UIViewController',
  {
    ...
    viewDidLoad: function() {
      ...
      if (typeof MNUtil !== "undefined" && MNUtil.readCloudKey) {
        ...
        if (savedConfig) {
          try {
            self.config = Object.assign(defaultConfig, JSON.parse(savedConfig));
            MNUtil.log("📱 加载已保存的配置");
          } catch (e) {
      ...
      // === 创建自动处理指示器（现在 config 已经初始化） ===
      if (typeof MNButton !== "undefined") {
        self.autoProcessIndicator = MNButton.new({
          title: "自动: " + (self.config.autoProcess ? "开" : "关"),
          font: 12,
          color: self.config.autoProcess ? "#4CAF50" : "#00000020",
          radius: 12,
          opacity: 0.8
        }, self.titleBar);
    ...
    // === 配置保存方法 ===
    saveConfig: function() {
      if (typeof MNUtil !== "undefined" && MNUtil.setCloudKey) {
        MNUtil.setCloudKey("SimplePanel_Config", JSON.stringify(self.config));
        MNUtil.log("💾 配置已保存到 iCloud");
      }
    },

...
每个任务卡片包含多个字段：
- **信息**：主字段，包含任务的基本信息
- **包含**：非动作类型任务的子任务容器（按状态分组）
- **启动**：动作类型任务的启动方式（应用/URL）
- **进展**：记录任务执行进展和里程碑，所有任务类型都包含此字段（v0.12.1新增）
- **所属**：链接到父任务
- **状态子字段**：📅 今日、🔥 优先级、⏰ 计划时间等
...
从 v0.12.2 开始，MNTask 新增了时间戳记录功能，让您可以快速为任务添加带时间戳的记录。

### 核心功能

#### 时间戳记录
- **精确时间记录**：自动获取当前时间，精确到秒
- **美观样式设计**：蓝色渐变时钟图标，视觉效果优雅
- **快速添加**：单击"记录"按钮即可添加
- **简单易用**：输入内容后自动添加到任务卡片末尾
...
记录会以特殊样式显示：
- 左侧显示蓝色渐变的时钟图标
- 时间戳以蓝色文字显示（格式：YYYY-MM-DD HH:MM:SS）
- 记录内容显示在时间戳下方
...
| 特性 | 时间戳记录 | 任务记录系统 |
|-----|----------|------------|
| 使用复杂度 | 简单，一键添加 | 需要填写进度等信息 |
| 时间格式 | 精确到秒 | 精确到分钟 |
| 进度追踪 | 不支持 | 支持百分比进度 |
| 适用场景 | 快速记录、会议纪要 | 项目进度管理 |

### 注意事项

- 时间戳记录会作为 HTML 格式的评论保存
- 每条记录独立显示，不会合并
- 记录内容支持 Markdown 格式
- 建议用于需要精确时间记录的场景
...
### v0.12.4 (2025-01-16) - 修复任务制卡清理失效链接问题
**修复了任务制卡功能对已有任务卡片不清理失效链接的问题**

#### 🐛 Bug 修复
- **任务制卡失效链接清理**
  - 修复了对已有任务卡片执行任务制卡时不会清理失效链接的问题
  - 现在无论是新制作任务卡片还是处理已有任务卡片，都会自动清理失效链接
  - 处理结果会显示清理的失效链接数量

#### 🛠 技术改进
- **processExistingTaskCards 函数优化**
  - 在处理已有任务卡片时增加了 cleanupBrokenLinks 调用
  - 如果清理了失效链接，会将结果类型标记为 'upgraded'
  - 改进了处理结果的信息反馈

- **convertToTaskCard 函数增强**  
  - 在新卡片转换过程中也添加了失效链接清理步骤
  - 确保所有任务卡片都保持整洁

### v0.12.1 (2025-01-15) - 新增任务进展字段
**为任务卡片增加了"进展"主字段，用于记录任务的执行进展和重要里程碑**

#### 🎉 新增功能
- **任务进展字段**
  - 为所有任务类型（目标、关键结果、项目、动作）添加"进展"主字段
  - "进展"字段位于任务卡片的最下方
  - 对于动作类型：位于"启动"字段之后
  - 对于其他类型：位于状态字段之后
  - 用于记录任务的关键进展、里程碑和重要更新
  - 支持富文本格式，可添加链接、时间戳等信息

- **旧卡片自动升级**
  - 执行 taskCardMake 时自动检测旧卡片
  - 对于缺少"进展"字段的旧任务卡片，自动添加该字段
  - 升级过程不影响现有字段和数据
  - 确保所有任务卡片具有统一的字段结构
...
- **字段结构优化**
  - 统一了所有任务类型都包含"进展"字段
  - 动作类型字段顺序：信息 → 启动 → 进展
  - 其他类型字段顺序：信息 → 包含 → 状态字段（未开始/进行中/已完成/已归档） → 进展
  - 改进了字段插入逻辑，确保新字段添加在正确位置
...
#### 📝 使用说明
- **添加进展记录**：在"进展"字段下方添加评论，记录任务的重要进展
- **时间戳格式**：建议使用 `[2025-01-15]` 格式标记进展时间
- **里程碑标记**：可使用 emoji 标记重要里程碑，如 `🚀 启动`、`✅ 完成第一阶段`

### v0.12.0 (2025-01-12) - 重大架构更新
**这是一个重大的架构更新版本，全面重构了看板功能的实现方式**

#### 🎉 新增功能
- **全新的开发文档体系**
  - 创建《MNTask 插件与网页交互开发指南》（/docs/webview-plugin-guide.md）
  - 提供从基础概念到实战案例的完整教程
  - 包含 Hello World 示例、看板功能实战、调试技巧等章节
  - 对网页开发新手友好，深入浅出的讲解方式

- **通信桥接库（mntask-bridge.js）**
  - 封装了 WebView 与插件之间的所有通信逻辑
  - 提供简单易用的 API（send、receive、on、once 等）
  - 支持 Promise 和回调两种调用方式
  - 内置错误处理、超时机制、性能统计
  - 完整的 JSDoc 文档和类型定义

- **示例代码库**
  - 01-hello-world.html：最简单的交互示例
  - 更多示例持续添加中

- **新的看板实现（todayboard_new.html）**
  - 去除 iframe 架构，直接加载 HTML
  - 更清晰的代码结构和注释
  - 更好的性能和响应速度
  - 支持网格和列表两种视图模式
  - 优化的移动端适配
...
#### 🔧 后续计划
- 将新架构应用到其他视图（项目视图、任务队列等）
- 完善示例代码库
- 优化插件端的 URL Scheme 处理
- 添加更多高级功能示例

**注意**：这个版本虽然是重大更新，但保持了向后兼容性。旧的看板功能仍然可用，新功能将逐步替换旧实现。

### v0.11.19 (2025-01-12)
- **修复**：iPad 上 iframe postMessage 通信时序问题
  - 增强消息重试机制，确保 iframe 内容能够接收到数据
  - 使用递增延迟（0ms, 100ms, 300ms, 600ms, 1000ms）多次发送消息
  - 添加调试日志，便于追踪消息传递流程
- **优化**：统一处理所有数据加载函数（loadTasksFromPlugin、loadProjectsFromPlugin、loadProjectTasksFromPlugin）
- **改进**：增强错误处理，返回更详细的错误信息

### v0.11.18 (2025-01-12)
- 修复 iPad 上 WebView JavaScript 执行返回 NSNull 的问题
- 添加 NSNull 检测机制，避免"result.startsWith is not a function"错误
- 优化 JavaScript 执行结果处理，提升 iPad 兼容性
- 修复 iPad 上所有视图显示"加载失败"的问题

### v0.11.16 (2025-01-12)
- **修复**：任务编辑器保存功能失败问题
  - 修复 taskEditor 对象未暴露到全局 window 导致原生代码无法访问的问题
  - 修复 handleSaveTaskChanges 中错误的空值检查（返回字符串 'null' 而非实际 null）
  - 增强 getChangedFields 返回值的日志记录
  - 优化 loadTaskDetailFromPlugin 的加载时机，添加重试机制
  - 移除 taskeditor.html 初始化时自动请求任务数据的逻辑，改为等待原生代码调用
  - 现在任务编辑器可以正常保存字段更改并同步到任务卡片
...
- mntask_guide.md: 添加 v0.11.3 更新说明

## 版本 v0.11.2

### 修复的问题

1. **看板按钮显示"Not supported yet"问题**
   - 修改了 `xdyy_custom_actions_registry.js` 中的 `openTodayBoard` action
   - 使用 `MNUtil.postNotification("openTaskSetting", {})` 通知系统打开设置面板
   - 然后通过 `MNTaskGlobal.mainPlugin` 访问 settingController 切换到看板视图

2. **项目视图筛选框显示问题**
   - 在 `projectview.html` 中将筛选侧边栏设置为默认收起状态 (添加 `collapsed` 类)
   - 增大筛选按钮尺寸 (36x40px)
   - 将按钮图标改为搜索图标 🔍
   - 添加悬浮效果和阴影，提高可见性

3. **今日看板重复显示"已修复旧版标记"**
   - 在 `settingController.js` 的 `handleRefreshBoard` 函数中移除了 `fixLegacyTodayMarks` 调用
   - 旧版标记修复应该是一次性操作，不应在每次刷新时执行
   - 添加"刷新完成"提示改善用户体验

4. **任务队列"今日"标签点击失败**
   - 修复了 `moveTaskToToday` 函数中调用不存在的 `TaskFieldUtils.updateFieldContent` 方法
   - 使用正确的方法来添加或更新日期字段
   - 检查是否已有日期字段，有则更新，无则添加
   - 确保日期字段格式统一为"📅 日期: YYYY-MM-DD"

### 代码改动详情

1. **xdyy_custom_actions_registry.js**
   ```javascript
   // 修改 openTodayBoard action
...
2. **projectview.html**
   - 筛选侧边栏添加默认 `collapsed` 类
   - 筛选按钮样式优化

3. **settingController.js**
   - `handleRefreshBoard`: 移除旧版标记修复调用
   - `moveTaskToToday`: 重写函数逻辑，使用正确的 API

### 测试要点

1. 点击看板按钮应该能正常打开任务看板
2. 项目视图的筛选按钮应该始终可见
3. 刷新看板不再重复显示修复旧版标记的提示
4. 任务队列中点击"今日"标签能成功添加日期字段

### 版本号更新

- mnaddon.json: 0.11.1 → 0.11.2
- mntask_guide.md: 添加 v0.11.2 更新说明
## 版本 v0.11.8

### 修复的严重架构问题

1. **任务编辑器架构重构**
   - 移除了错误的独立浮动 WebView 实现
   - 恢复了正确的架构：任务编辑器在设置面板的 iframe 中显示
   - 修复了编辑任务时"正在打开任务编辑器..."但界面无变化的问题

### 技术改进

1. **正确的编辑流程实现**
   - editTask 方法现在通过 JavaScript 调用 sidebarContainer 的 showTaskEditor 函数
   - showTaskEditor 负责切换到任务编辑器视图并加载数据
   - 统一了 editingTaskId 的使用，避免变量名混乱

2. **修复数据通信**
   - handleSaveTaskChanges 现在正确从 iframe 中获取编辑数据
   - loadTaskDetailForEditor 正确发送数据到 iframe 中的编辑器
   - 移除了所有对不存在的 taskEditorWebView 的引用

### 实现细节

- 删除了 createTaskEditorWebView 和 loadTaskToEditor 方法
...
1. **iPad端所有视图显示"加载中"问题**
   - **问题**：WebView 执行 JavaScript 返回 null，导致数据无法加载
   - **修复**：改进 JavaScript 执行方法，使用包装函数确保返回值
...
2. **任务编辑器功能失效问题**
   ...
   - **修复**：
     - 改进任务数据加载机制
     - 使用 postMessage 方式获取编辑器数据
     - 修复字段更改的保存逻辑

3. **WebView JavaScript 执行优化**
   - 为所有数据加载方法添加错误处理和重试机制
   - 统一使用包装函数避免 NSNull 返回值
   - 改进 iframe 架构下的通信机制
...
1. **JavaScript 执行机制**
   - 使用 IIFE (立即执行函数表达式) 包装所有 JavaScript 代码
   - 添加 try-catch 错误处理
   - 确保始终返回有效值而非 null

2. **iframe 通信优化**
   - 任务编辑器支持 postMessage 方式获取数据
   - 改进父窗口与 iframe 之间的消息传递
   - 增强通信的可靠性和兼容性

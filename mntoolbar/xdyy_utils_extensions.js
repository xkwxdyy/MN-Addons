...
function initXDYYExtensions() {
  ...
  // 修改卡片前缀
  toolbarUtils.makeCardsAuxChangefocusNotePrefix = function(focusNote, parentNote) {
    let newTitle
    let focusNoteColorIndex = focusNote.note.colorIndex
  ...
  toolbarUtils.makeCardsAuxLinkToParentNote = function(focusNote, focusNoteType, parentNote) {
    /*
      【focusNote 为非黄色卡片时】
        父卡片为：
          归类型的淡黄色卡片：双向链接
  ...
  // 将"相关概念："移动到下方
  toolbarUtils.makeCardsAuxMoveDownDefinitionsComments = function(focusNote) {
  ...
  toolbarUtils.makeCardsAuxMoveDownApplicationsComments = function(focusNote) {
    let applicationHtmlCommentIndex = focusNote.getCommentIndex("应用：",true)
  ...
  toolbarUtils.moveProofToMethod = function(focusNote,methodNum) {
    let focusNoteComments = focusNote.note.comments
    let focusNoteCommentLength = focusNoteComments.length
    let nonLinkNoteCommentsIndex = []
    let proofHtmlCommentIndex
    ...
    for (let i = focusNoteCommentLength-1; i >= nonLinkNoteCommentsIndex[0]; i--) {
      focusNote.moveComment(focusNoteCommentLength-1, proofHtmlCommentIndex);
    }
  }
   ...
   * 夏大鱼羊
   */
  toolbarUtils.getKnowledgeNoteTypeByColorIndex = function(colorIndex) {
    let focusNoteType
    switch (colorIndex) {
      case 0: // 淡黄色
        focusNoteType = "classification"
   ...
   * 夏大鱼羊
   */
  toolbarUtils.referenceMoveLastCommentToThought = function(focusNote){
   ...
   * 夏大鱼羊
   */
  toolbarUtils.moveLastCommentToProof = function(focusNote){
   ...
   * 夏大鱼羊
   */
  toolbarUtils.moveLastTwoCommentsToThought = function(focusNote){
   ...
   * 夏大鱼羊
   */
  toolbarUtils.moveLastTwoCommentsToProof = function(focusNote){
    focusNote.moveCommentsByIndexArrTo([focusNote.comments.length-1], "proof")
    focusNote.moveCommentsByIndexArrTo([focusNote.comments.length-1], "proof")
  }
   ...
   * 夏大鱼羊
   */
  toolbarUtils.referenceMoveLastTwoCommentsToThought = function(focusNote){
    let refedHtmlCommentIndex = focusNote.getCommentIndex("被引用情况：", true)
    focusNote.moveComment(focusNote.comments.length-1, refedHtmlCommentIndex)
    focusNote.moveComment(focusNote.comments.length-1, refedHtmlCommentIndex)
  }
   ...
   * 夏大鱼羊
   */
  toolbarUtils.addThought = function(focusNotes) {
   ...
   * 夏大鱼羊
   */
  toolbarUtils.referenceAddThoughtPoint = function(focusNote) {
    let thoughtHtmlCommentIndex = focusNote.getCommentIndex("相关思考：", true)
    if (thoughtHtmlCommentIndex !== -1) {
      let refedHtmlCommentIndex = focusNote.getCommentIndex("被引用情况：", true)
   ...
   * 夏大鱼羊
   */
  toolbarUtils.moveUpThoughtPoints = function(focusNote) {
    let thoughtHtmlCommentIndex = focusNote.getCommentIndex("相关思考：", true)
    if (thoughtHtmlCommentIndex !== -1) {
      let keywordsHtmlCommentIndex = focusNote.getIncludingCommentIndex("关键词：", true)
      let linkHtmlCommentIndex = focusNote.getCommentIndex("相关链接：", true)
   ...
   * 夏大鱼羊
   */
  toolbarUtils.referenceMoveUpThoughtPoints = function(focusNote) {
    let thoughtHtmlCommentIndex = focusNote.getCommentIndex("相关思考：", true)
    if (thoughtHtmlCommentIndex !== -1) {

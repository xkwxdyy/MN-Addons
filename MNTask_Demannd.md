...
> “看板里任务卡片” || “html 里任务卡片”默认指的是“task-focus-board.html”文件中的任务卡片
> 如果需要查看以前的版本，可以解压以前的版本的 `.mnaddon` 后缀的文件，用 `mnaddon4 unpack <path>` 解压。
> 时刻记住 `/Users/xiakangwei/Nutstore/Github/repository/MN-addon-develop/MN-Addon/mnbrowser` 是重要的研究数据双向传输的参考，这是一个 MN 内的浏览器插件。


“task-focus-board.html” 是提前写好的，然后测试了一些数据之后加入了 MNTask 插件进行调用，然后会有一些兼容性的问题和交互问题需要解决，并且要考虑到软件和 html 的实际数据交互！

注意：我们开发的目的就是要让“task-focus-board.html” 来辅助 MN 的卡片编辑和筛选等，实现良好的任务管理！目前最重要的是实现 html 里的任务卡片数据和软件里任务卡片数据的双向修改同步。

目前已经实现的双向数据传输的功能（成功实践）
- “task-focus-board.html” 中“看板”视图的任务卡片可以定位 MN 中的任务卡片

我下面根据当前的情况来整理一些问题，请你仔细分析进行解决。


当前最最最严重的问题是 0532a0e7a00163051f1ea9fcfe87243740861959 和  75232e0f024a2867049d69db5d624cd5a24066c6 两次 git commit 之后，造成了“今日看板”打开失效。

13_31 版本，mac 和 iPad 都能打开今日看板。mac 上能调取到数据，iPad 上不能调取到数据，只能加载测试数据！0532a0e7a00163051f1ea9fcfe87243740861959 和  75232e0f024a2867049d69db5d624cd5a24066c6 两次 commit 一开始本来是为了解决上面的这个问题，结果越改越差！现在直接崩了！最新 14_5 版本的点击“今日看板”后的 log 文件已更新到 information.log 中。请你仔细分析，并修复今日看板面板打不开，或者说无法显示的问题，并且在 iPad 上能加载数据，而不是测试数据。

一个可能有帮助的观察是（13_31 版本就已经这样了）：现在 MNTask 的 setting 打开后，Button 和 Dynamic 下方的 jsoneditor.html 不能正常加载了！应该是和你的什么代码修改产生了严重的冲突！导致看板也崩了！
ultrathink
...
焦点卡片有一些问题
- 点击后无法弹出任务详情及其修改。这是最严重的问题，和最需要修复的特性！
- 原卡片的进展数据应该是没有成功解析？进展的数据这样
```
...
开始「评论提取出来，删除后可以支持把提出卡片制卡后链接到原卡片」
```
`2025-07-20 15:33` 是时间，`开始「评论提取出来，删除后可以支持把提出卡片制卡后链接到原卡片」` 是进展内容。最关键的就是 html 里要能获取到MN 中任务卡片里的进展，并且 html 里的添加要能够以同样的形式添加到 MN 中的任务卡片，目前 html 里添加的进展无法回到卡片本身！

ultrathink

## 打开今日看板的性能优化

现在可以通过插件菜单栏的 “今日看板” 打开 MNTask 看板，也就是“task-focus-board.html”。但是每次由于要遍历一遍任务卡片，所以会有好几秒的卡顿！软件内测试 mac 上鼠标直接转圈了的那种。并且软件内关掉之后再重新点击“今日看板”会重新遍历，感觉非常耗性能，能否进行一些优化？

比如打开笔记本后第一次打开看板的话，进行一次重新刷新，然后只要笔记本没有关，或者软件没重启的话，之后需要用户手动刷新才会完整刷新。其次就是现在是否里面有太多 MNUtil.log 信息占用了性能？
...
MNtask 里 xdyy_utils_extensions.js 开发任务卡片的暂停状态（不在“状态”的单击里切换，而是在长按菜单里增加暂停功能），用于“进行中”的任务，暂停后点击“状态”按钮的单击，可以再次变成“进行中”
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
2. 点击“启动”按钮的单击效果一直显示“未找到启动链接”，但启动字段里的链接我两种都试过了，仍然显示“未找到启动链接”，请分析并解决。
3. setting 里的“看板”视图先去掉吧，现在暂时不需要，先把控制显示的那个先注释掉，否则会[
   {
   "version": "marginnote4",
   "type": "macOS",
   "taskVersion": "0.12.5"
...
2. 从按钮点击看板，打开看板后仍然显示“打开看板失败”！请解决！
   “任务队列”界面
3. 任务的“今日”按钮点击无效
4. 点击完成后，脑图里的卡片状态并没有更新，然后弹出了一个报错
   ultrathink

我们刚刚增加了一个搜索的按钮，现在我们重点在 MNMath 里结合开发一下搜索的功能，放在按钮里。
1. 范围：我们的搜索只在知识库的子孙卡片内容，知识库的根目录的卡片 ID 为 B2A5D567-909C-44E8-BC08-B1532D3D0AA1。
2. 刚刚的 showDefinitionCatalog 函数变成放在长按菜单里，单击触发一般的搜索。

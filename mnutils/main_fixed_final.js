 ...
 * 
 * 主要模块：
 * - MNSubscription: 插件生命周期管理
 * - subscriptionController: UI 控制器
 * - subscriptionUtils: 工具函数
 * - subscriptionNetwork: 网络请求
 * - subscriptionConfig: 配置管理
 * 
 * @version 0.1.5.alpha0624
 * @optimized 2025-08-09
 */


// ========== 常用常量（供参考，未自动替换） ==========
// 使用这些常量可以提高代码可读性，但需要手动替换
/*
const COLORS = {
  PRIMARY: '#457bd3',
  SUCCESS: '#75fb4c',
...
(void 0 === MNOnAlert && (MNOnAlert = !1),
  (JSB.newAddon = function (t) {
    try {
      ...
      subscriptionUtils.showHUD('Error when loading mnutils: ' + error);
    }
    JSB.require('mnutils');
    try {
      JSB.require('CryptoJS');
    } catch (error) {
      MNUtil.log('Error when loading CryptoJS: ' + error);
    }
    try {
      JSB.require('marked');
    } catch (error) {
      MNUtil.log('Error when loading marked: ' + error);
    }
    try {
      JSB.require('pdf');
    } catch (error) {
      MNUtil.log('Error when loading pdf: ' + error);
    }
    try {
      JSB.require('highlight');
    } catch (error) {
      MNUtil.log('Error when loading highlight: ' + error);
    }
    try {
      JSB.require('mustache');
    } catch (error) {
      MNUtil.log('Error when loading mustache: ' + error);
    }
    try {
      JSB.require('segmentit');
    } catch (error) {
      MNUtil.log('Error when loading segmentit: ' + error);
    }
    try {
      JSB.require('jsonrepair');
    } catch (error) {
      MNUtil.log('Error when loading jsonrepair: ' + error);
    }
      ...
      'MNSubscription : JSExtension',
      {
        sceneWillConnect: async function () {
          try {
            ...
            subscriptionUtils.addErrorLog(error, 'sceneWillConnect');
          }
        },
        ...
        onAddonBroadcast: async function (broadcast) {
          const url = 'marginnote4app://addon/' + broadcast.userInfo.message,
            e = MNUtil.parseURL(url),
            addonName = e.pathComponents[0];
          if ('mnutils' === addonName)
            switch (e.params.action) {
              case 'changeView':
                subscriptionUtils.subscriptionController.changeViewTo(e.params.view);
                break;
              case 'loadRechargePage':
                const i = subscriptionUtils.subscriptionController;
        ...
        notebookWillOpen: function (t) {
          try {
            ...
            subscriptionUtils.addErrorLog(error, 'notebookWillOpen');
          }
        },
        ...
        controllerWillLayoutSubviews: function (t) {
          if (t === MNUtil.studyController)
            try {
              let e = subscriptionUtils.subscriptionController;
              if (e && !e.onAnimate && !e.view.hidden) {
                const i = MNUtil.currentWindow.bounds;
              ...
              subscriptionUtils.addErrorLog(error, 'controllerWillLayoutSubviews');
            }
        },
        ...
        toggleAddon: async function (t) {
          try {
            ...
            subscriptionUtils.showHUD(error);
          }
        },
      },
      {
  ...
  'subscriptionController : UIViewController <NSURLConnectionDelegate,UIWebViewDelegate>',
  {
    viewDidLoad: function () {
      try {
        ...
        subscriptionUtils.addErrorLog(error, 'viewDidLoad');
      }
    },
    viewWillAppear: (t) => {},
    viewWillDisappear: (t) => {},
    ...
    connectionDidReceiveData: async function (t, e) {
      try {
        ...
        subscriptionUtils.addErrorLog(error, 'connectionDidReceiveData');
      }
    },
    connectionDidFinishLoading: async function (t) {
      try {
        if (((self.onDownloading = !1), !(400 <= self.statusCode))) {
          if (self.targetPath)
            switch ((self.currentData.writeToFileAtomically(self.targetPath, !1), self.fileType)) {
              case 'document':
                const e = MNUtil.getFileName(self.targetPath),
        ...
        (MNUtil.stopHUD(), subscriptionUtils.addErrorLog(error, 'connectionDidFinishLoading'));
      }
    },
    ...
    webViewDidFinishLoad: async function (t) {
      const e = getSubscriptionController();
      if (!t.sidebar) {
        var i,
    ...
    webViewShouldStartLoadWithRequestNavigationType: function (t, e, i) {
      try {
        ...
        return (subscriptionUtils.addErrorLog(error, 'webViewShouldStartLoadWithRequestNavigationType'), !1);
      }
      return !0;
    },
    ...
    chooseAPIKeyForQuota: async function () {
      const t = getSubscriptionController(),
    ...
    chooseQuota: async function () {
      const t = getSubscriptionController(),
        e = subscriptionConfig.APIKey;
      if (e) {
        const i = await MNUtil.userSelect('Choose the recharge quota', '选择要增加的额度\n(充值到账有数小时延迟)', [
            '5 Points (5¥)',
            '10 Points (9.5¥)',
            '20 Points (19¥)',
            '50 Points (45¥)',
    ...
    changeView: async function (t) {
      ...
      try {
        ...
        subscriptionUtils.addErrorLog(error, 'changeView');
      }
    },
    setViewTo: async function (t) {
      ...
      try {
        ...
        subscriptionUtils.addErrorLog(error, 'setViewTo');
      }
    },
    ...
    showDetail: async function (t) {
      await self.checkDocview();
      const e = subscriptionConfig.APIKey;
      e
    ...
    activate: async function (t = 1) {
      self.popoverController && self.popoverController.dismissPopoverAnimated(!0);
      let e = self.apikeyInput.text.trim();
      if (
    ...
    toggleAutoSubscription: async function (t) {
      try {
        const e = subscriptionConfig.getConfig('autoSubscription');
        ...
        subscriptionUtils.addErrorLog(error, 'toggleAutoSubscription');
      }
    },
    ...
    refreshButtonTapped: async function (t) {
      ...
      switch (subscriptionConfig.lastView) {
        case 'subscriptionView':
          const i = await subscriptionNetwork.getUsage();
          return (
            'error' in i
    ...
    onMoveGesture: function (t) {
      try {
        ...
        if (1 === t.state)
          ...
          const c = subscriptionUtils.topOffset,
          ...
          if (i.left < 20 && i.top < 20) subscriptionConfig.config.customMode = 'leftTop';
            ...
            const d = Object.keys(i).reduce((t, e) => (i[t] < i[e] ? t : e));
          ...
          ((self.onAnimate = !0),
            MNUtil.animate(() => {
              ...
              ((self.onAnimate = !1),
                ...
                subscriptionConfig.save(),
                self.setMiniColor(!0));
              const t = self.lastFrame;
              self.lastFrame = subscriptionUtils.calcLastFrame(t);
            }));
        }
      } catch (error) {
        subscriptionUtils.addErrorLog(error, 'onMoveGesture');
      }
    },
...
function tryCatch(e) {
  return function (...t) {
    try {
      ...
      return (MNUtil.showHUD(error), null);
    }
  };
}
((subscriptionController.prototype.refresh = async function (t = !0) {
  try {
    ...
    subscriptionUtils.addErrorLog(error, 'refresh');
  }
}),
  (subscriptionController.prototype.init = async function () {
    try {
      ...
      subscriptionUtils.addErrorLog(error, 'init');
    }
  }),
  ...
  (subscriptionController.prototype.runJavaScript = async function (i, o = 'webview') {
    return new Promise((e, t) => {
      try {
        ...
        (subscriptionUtils.addErrorLog(error, 'runJavaScript'), e(0));
      }
    });
  }),
  ...
  (subscriptionController.prototype.refreshSidebar = async function (e = !0, r = 'webview') {
    try {
      ...
      subscriptionUtils.addErrorLog(error, 'refreshSidebar');
    }
  }),
  ...
  (subscriptionController.prototype.activate = async function (t = 1) {
    ...
    const e = this.apikeyInput.text.trim();
  ...
  (subscriptionController.prototype.toMinimode = function (i = !0) {
    try {
      ...
      subscriptionUtils.addErrorLog(error, 'toMinimode');
    }
  }),
  ...
  (subscriptionController.prototype.checkDocview = async function () {
    try {
      ...
      subscriptionUtils.addErrorLog(error, 'checkDocview');
    }
  }),
  ...
  (subscriptionController.prototype.afdNewkeyOrderPage = async function (t, e = !1) {
    let index = ['5', '10', '20', '50'].indexOf(t),
    ...
    (MNUtil.copy(i), e ? MNUtil.openURL(orderUrl) : MNConnection.loadRequest(this.docview, orderUrl, !0));
  }),
  (subscriptionController.prototype.afdRechargeOrderPage = async function (t, e = !1) {
    const index = ['5', '10', '20', '50'].indexOf(t),
      i = [
        'https://ifdian.net/order/create?product_type=1&plan_id=f52724aa464611ef8acc52540025c377&sku=%5B%7B%22sku_id%22:%22f52fdfc8464611efbe3052540025c377%22,%22count%22:1%7D%5D',
        'https://ifdian.net/order/create?product_type=1&plan_id=f52724aa464611ef8acc52540025c377&sku=%5B%7B%22sku_id%22:%22a077cf88557611efb1785254001e7c00%22,%22count%22:1%7D%5D',
        'https://ifdian.net/order/create?product_type=1&plan_id=f52724aa464611ef8acc52540025c377&sku=%5B%7B%22sku_id%22:%22a0831816557611ef8fb05254001e7c00%22,%22count%22:1%7D%5D',
  ...
  (subscriptionController.prototype.executeMNAddonAction = async function (t) {
    try {
      ...
      subscriptionUtils.addErrorLog(error, 'executeMNAddonAction');
    }
  }),
  (subscriptionController.prototype.changeView = async function (t) {
    const e = subscriptionConfig.lastView,
  ...
  (subscriptionController.prototype.changeViewTo = async function (t, e = !1) {
    try {
      ...
      subscriptionUtils.addErrorLog(error, 'changeViewTo');
    }
  }),
      ...
      new Promise((t, e) => {
        ((this.onAnimate = !0),
          MNUtil.animate(() => {
            ...
            const t = this.view.bounds.width;
  ...
  (subscriptionController.prototype.testURL = async function (e, i) {
    ...
    try {
      ...
      this.waitHUD('Testing URL ' + 20 * this.testIndex + '%...');
      i = '❌ ' + URLName;
      return { success: !1, error: error.message, timeUsed: -1, name: URLName, url: e, title: i };
    }
  }));
class subscriptionUtils {
  ...
  static getByDefault(t, e) {
    const i = NSUserDefaults.standardUserDefaults().objectForKey(t);
  ...
  static currentNotebook() {
    const t = this.studyController().notebookController.notebookId;
  ...
  static refreshAddonCommands() {
    const t = subscriptionUtils.subscriptionController;
    t &&
      t.miniMode &&
  ...
  static getPopoverAndPresent(t, e, i = 100, o = 2) {
    const s = MenuController.new(),
      popover =
        ((s.commandTable = e),
        (s.rowHeight = 35),
        (s.preferredContentSize = { width: i, height: s.rowHeight * s.commandTable.length }),
        new UIPopoverController(s)),
      rect = t.convertRectToView(t.bounds, this.studyView());
    return (popover.presentPopoverFromRect(rect, this.studyView(), o, !0), popover);
  }
  ...
  static compareVersions(t, e) {
    ...
    for (let t = 0; t < Math.max(o.length, s.length); t++) {
      let n = o[t] || { num: 0, alpha: '', alphaNum: 0 },
  ...
  static getRandomAuthorization(t, e = !1) {
    t = 'https://dav.jianguoyun.com/dav/mnaddonStore/' + t;
    let i = [
  ...
  static async importNotebook(o, s, t) {
    try {
      const n = subscriptionUtils.subscriptionController,
      ...
      if (
        ...
        const d = MNUtil.subpathsOfDirectory(s + '/').filter((t) => t.endsWith('.pdf'));
        let i = d.map((t) => s + '/' + t);
        const u = d.map((t) => MNUtil.documentFolder + '/' + t);
        ...
        switch (((t = t.filter((t) => t.endsWith('.marginnotes'))), e)) {
          ...
          case 1:
            (n.waitHUD('Importing notebook...'), await MNUtil.delay(0.1));
            const h = MNUtil.importNotebook(s + '/' + t[0], !0);
            (await MNUtil.delay(0.1),
              u.length &&
          ...
          case 3:
            (n.waitHUD('Importing notebook...'), await MNUtil.delay(0.1));
            const p = MNUtil.importNotebook(s + '/' + t[0], !1);
            (await MNUtil.delay(0.1),
              u.length &&
                (n.waitHUD('Importing documents...'),
                u.forEach((t, e) => {
      ...
      subscriptionUtils.addErrorLog(error, 'importNotebook');
    }
  }
  ...
  static calcLastFrame(e = void 0) {
    try {
      ...
      const i = subscriptionConfig.getConfig('customMode'),
      ...
      subscriptionUtils.addErrorLog(error, 'calcLastFrame');
    }
  }
}
class subscriptionNetwork {
  ...
  static initRequest(t, e) {
    const i = NSMutableURLRequest.requestWithURL(this.genNSURL(t));
  ...
  static getJSONData(t) {
    try {
      ...
      subscriptionUtils.addErrorLog(error, 'getJSONData');
    }
  }
    ...
    return new Promise((l, t) => {
      NSURLConnection.sendAsynchronousRequestQueueCompletionHandler(e, i, (t, e, i) => {
        try {
          ...
          l({ error: error.localizedDescription || 'Unknown error' });
        }
      });
    });
  }
  ...
  static async subscribe(e = 1, i = !0) {
    try {
      ...
      this.onSubscrbing = !0;
      const o = { 'Content-Type': 'application/json', Authorization: 'Bearer ' + subscriptionConfig.config.apikey };
      let t = 'mnaddon';
      const s = { model: (t = 1 < e ? 'mnaddon' + e : t), messages: [{ role: 'user', content: 'mnaddon' }] },
      ...
      return (subscriptionUtils.addErrorLog(error, 'subscribe'), MNUtil.stopHUD(), { success: !1, error: error });
    }
  }
  ...
  static async countDownload(t = 'mnaddon') {
    let e = {
      ...
      t_2 = { model: t_2, messages: [{ role: 'user', content: 'mnaddon' }] },
  ...
  static async getUsage(t = subscriptionConfig.URL, e = subscriptionConfig.APIKey) {
    try {
      const i = t + '/v1/dashboard/billing/subscription',
        o = t + '/v1/dashboard/billing/usage';
      if (e && '' !== e.trim()) {
        const s = { Authorization: 'Bearer ' + e, 'Content-Type': 'application/json' },
          n = {},
        ...
        if ((MNUtil.log({ message: 'getUsage:' + t, detail: r }), 'error' in r)) {
          ...
          const c = await this.fetch(i, { method: 'GET', timeout: 10, headers: s }),
      ...
      return (subscriptionUtils.addErrorLog(error, 'getUsage'), { error: error.message });
    }
  }
  static async validateAPIKey(e) {
    try {
      const i = subscriptionConfig.URL + '/api/token/search?keyword=&token=' + e;
      ...
      return (subscriptionUtils.addErrorLog(error, 'validateAPIKey'), { success: !1, error: error.message() });
    }
  }
  static btoa(t) {
  ...
  static readWebDAVFileWithDelegate(t, e, i) {
    try {
      const o = { Authorization: 'Basic ' + this.btoa(e + ':' + i), 'Cache-Control': 'no-cache' };
  ...
  static async downloadFromConfig(t, e = void 0) {
    ...
    if ('importDocument' === t.action)
      ...
      const o = MNUtil.getNoteBookById(t.id);
      if (o && o.notes?.length)
        switch (
      ...
      let s = t.url.endsWith('.marginpkg') ? t.id + '.marginpkg' : t.id + '.marginnotes',
  ...
  static async downloadFromConfigReTry(t = void 0) {
    let e = subscriptionUtils.preAddonConfig,
    ...
    if ('importDocument' === e.action)
      ...
      const o = MNUtil.getNoteBookById(e.id);
      if (o && o.notes?.length)
        switch (
          await MNUtil.userSelect('Notebook already exists', '学习集已存在，是否打开该学习集？\n\n' + e.name, [
            '打开学习集',
      ...
      let s = e.url.endsWith('.marginpkg') ? e.id + '.marginpkg' : e.id + '.marginnotes',
        n = subscriptionUtils.downloadPath + '/' + s;
      (MNUtil.isfileExists(n)
...
class subscriptionConfig {
  ...
  static addSubscriptionDay(t) {
    const e = subscriptionConfig.getConfig('subscriptionDaysRemain');
    this.config.subscriptionDaysRemain = e + t;
  }
  /**
   * 检查用户是否已订阅
   * @returns {boolean} true 如果已订阅且在有效期内
   */
  static isSubscribed() {
    ...
    const today = subscriptionUtils.getToday();
    const subscribedDay = this.config.subscribedDay;
    ...
    // 计算订阅天数差
    const daysDiff = subscriptionUtils.calculateDaysDifference(subscribedDay, today);
  ...
  /**
   * 获取每日免费使用次数（用 emoji 显示）
   * 每天重置为 10 次，使用数字 emoji 倒计时显示剩余次数
   * @returns {string} 剩余次数的 emoji (🔟 到 0️⃣)
   */
  static getFreeUsage() {
    const today = subscriptionUtils.getToday();
    
    // 检查是否是同一天
    if (this.getConfig('freeDay') === today) {
      // 使用数组索引映射剩余次数到对应的 emoji
      // 索引 0 = 🔟(10次), 索引 1 = 9️⃣(9次), ... 索引 10 = 0️⃣(0次)
      const remainingEmojis = ['🔟', '9️⃣', '8️⃣', '7️⃣', '6️⃣', '5️⃣', '4️⃣', '3️⃣', '2️⃣', '1️⃣', '0️⃣'];
      const usageIndex = this.getConfig('freeUsage');
      return remainingEmojis[usageIndex] || '0️⃣';
    } else {
      // 新的一天，重置免费使用次数
      this.config.freeDay = today;
  ...
  static getStatus() {
    if (this.isSubscribed()) {
      const remainingDays = 30 - subscriptionUtils.calculateDaysDifference(
        this.config.subscribedDay, 
        subscriptionUtils.getToday()
      );
      return `Activated (剩余 ${remainingDays} 天)`;
    } else if (this.config.subscriptionDaysRemain > 0) {
      return `Unactivated (可用 ${this.config.subscriptionDaysRemain} 天)`;
    } else {
      return 'Unactivated';
    }
  }

...
### 任务描述字段保存问题（2025-01-05 已修复）

**问题描述**：
任务详情中输入的描述在关闭模态框后会丢失，刷新页面或切换视图后描述字段消失。

**根本原因**：
存在一个有问题的 `useEffect` 会在 `tasks` 或 `pendingTasks` 更新时重新计算 `allTasks`，但它使用了闭包中的旧 `allTasks` 值，导致刚保存的更新被覆盖。

**解决方案**：
1. 移除了有问题的 allTasks 同步 useEffect（第504-525行）
2. 确保所有任务操作函数（`addToPending`、`addTaskToPending` 等）都显式更新 `allTasks`
3. 添加详细的调试日志以追踪数据流

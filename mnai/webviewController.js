...
try {
  ...
  try {
    ...
    if (systemMessage) {
      ...
      MNUtil.showHUD("Empty system message")
    }
  ...
  toggleAutoClear:function (params) {
    ...
    if (self.popoverController) {self.popoverController.dismissPopoverAnimated(true);}
      ...
      chatAIConfig.save('MNChatglm_config')
      MNUtil.showHUD("Auto clear input: "+(autoClear?"❌":"✅"))
  },
...
try {
  ...
  this.createButton("importSubscriptionButton","importFromSubscription:","modelView")
  MNButton.setTitle(this.importSubscriptionButton, "Import from subscription")
  MNButton.setColor(this.importSubscriptionButton, "#457bd3",0.8)
...
chatglmController.prototype.setModel = function (source) {
  ...
  switch (source) {
    ...
    case "ChatGPT":
      ...
      this.importSubscriptionButton.frame = MNUtil.genFrame(realX, 285, realWidth-10, 35)
      break;
    case "Subscription":
...
chatglmController.prototype.setModelButton = function (source) {
  if (source == "ChatGPT") {
    ...
    if (typeof subscriptionConfig === 'undefined') {
      this.importSubscriptionButton.hidden = true
    }else{
      this.importSubscriptionButton.hidden = !chatAIUtils.checkSubscribe(false)
    }
  }else{
 ...
 * @this {chatglmController}
 */
chatglmController.prototype.loadWebviewContent = function () {
  this.webviewInput.loadFileURLAllowingReadAccessToURL(
    NSURL.fileURLWithPath(this.mainPath + '/jsoneditor.html'),

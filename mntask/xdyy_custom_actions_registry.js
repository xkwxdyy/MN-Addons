...
function registerAllCustomActions() {
      ...
      (alert, buttonIndex) => {
        try {
          MNUtil.undoGrouping(() => {
            ...
            if (buttonIndex == 1) {
              let refNote = taskUtils.referenceRefByRefNum(focusNote, refNum)[0];
              let classificationNote = taskUtils.referenceRefByRefNum(focusNote, refNum)[1];
      ...
      (alert, buttonIndex) => {
        try {
          MNUtil.undoGrouping(() => {
            let refNum = alert.textFieldAtIndex(0).text;
            if (buttonIndex == 1) {
              taskUtils.referenceRefByRefNum(focusNote, refNum);
            }
          });
        } catch (error) {
          MNUtil.showHUD(error);
        ...
        try {
          MNUtil.undoGrouping(() => {
            if (buttonIndex == 1) {
              ...
              if (
                taskConfig.referenceIds.hasOwnProperty(currentDocmd5) ||
                taskConfig.referenceIds[currentDocmd5][0] == undefined
              ) {
                ...
                if (taskConfig.referenceIds[currentDocmd5].hasOwnProperty(refNum)) {
                  MNUtil.showHUD(
                    "[" +
                      refNum +
                      "] 与「" +
                      MNNote.new(taskConfig.referenceIds[currentDocmd5][refNum]).noteTitle +
                      "」绑定",
                  );
                } else {
                  MNUtil.showHUD("[" + refNum + "] 未进行 ID 绑定");
      ...
      (alert, buttonIndex) => {
        try {
          MNUtil.undoGrouping(() => {
            ...
            if (buttonIndex == 1) {
              taskUtils.referenceStoreOneIdForCurrentDoc(input);
            }
          });
        } catch (error) {
          MNUtil.showHUD(error);
  ...
  MNTaskGlobal.registerCustomAction("referenceStoreIdsForCurrentDoc", async function (context) {
    ...
    UIAlertView.showWithTitleMessageStyleCancelButtonTitleOtherButtonTitlesTapBlock(
      ...
      (alert, buttonIndex) => {
        try {
          MNUtil.undoGrouping(() => {
            let idsArr = taskUtils.splitStringByFourSeparators(alert.textFieldAtIndex(0).text);
            if (buttonIndex == 1) {
              idsArr.forEach((id) => {
                taskUtils.referenceStoreOneIdForCurrentDoc(id);
              });
            }
          });
        } catch (error) {
    ...
    UIAlertView.showWithTitleMessageStyleCancelButtonTitleOtherButtonTitlesTapBlock(
      ...
      (alert, buttonIndex) => {
        if (buttonIndex == 1) {
          try {
            MNUtil.undoGrouping(() => {
              let idsArr = taskUtils.splitStringByFourSeparators(MNUtil.clipboardText);
              idsArr.forEach((id) => {
                taskUtils.referenceStoreOneIdForCurrentDoc(id);
              });
            });
  ...
  MNTaskGlobal.registerCustomAction("referenceExportReferenceIdsToFile", async function (context) {
    ...
    MNUtil.writeText(path, JSON.stringify(taskConfig.referenceIds, null, 2));
    UTI = ["public.json"];
    ...
    UIAlertView.showWithTitleMessageStyleCancelButtonTitleOtherButtonTitlesTapBlock(
      ...
      (alert, buttonIndex) => {
        if (buttonIndex == 1) {
          try {
            MNUtil.undoGrouping(() => {
              taskConfig.referenceIds = JSON.parse(MNUtil.clipboardText);
              taskConfig.save("MNTask_referenceIds");
            });
          } catch (error) {
            MNUtil.showHUD(error);
  ...
  MNTaskGlobal.registerCustomAction("referenceInputReferenceIdsFromFile", async function (context) {
    ...
    try {
      ...
      taskConfig.referenceIds = MNUtil.readJSON(path);
      taskConfig.save("MNTask_referenceIds");
      // })
    } catch (error) {
      MNUtil.showHUD(error);
  ...
  MNTaskGlobal.registerCustomAction("referenceAuthorRenewAbbreviation", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        let authorName = taskUtils.getFirstKeywordFromTitle(focusNote.noteTitle);
        let abbreviations = taskUtils.getAbbreviationsOfName(authorName);
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        if (focusNote.excerptText) {
          taskUtils.convertNoteToNonexcerptVersion(focusNote);
        }
              ...
              UIAlertView.showWithTitleMessageStyleCancelButtonTitleOtherButtonTitlesTapBlock(
                ...
                (alertI, buttonIndexI) => {
                  if (buttonIndex == 1) {
                    ...
                    try {
                      taskUtils.referenceSeriesBookMakeCard(focusNote, seriesName, seriesNum);
                    } catch (error) {
                      MNUtil.showHUD(error);
                    }
                  }
                ...
                if (buttonIndex === 1) {
                  ...
                  for (let i = 0; i <= journalLibraryNote.childNotes.length - 1; i++) {
                    if (journalLibraryNote.childNotes[i].noteTitle.includes(journalName)) {
                      ...
                      journalName = taskUtils.getFirstKeywordFromTitle(targetJournalNote.noteTitle);
                      findJournal = true;
                    }
                  }
                  if (!findJournal) {
                  ...
                  if (journalInfoHtmlCommentIndex == -1) {
                    taskUtils.cloneAndMerge(focusNote, "1C976BDD-A04D-46D0-8790-34CE0F6671A4");
                  }
                  UIAlertView.showWithTitleMessageStyleCancelButtonTitleOtherButtonTitlesTapBlock(
                    "卷号",
                    "",
                    ...
                    (alertI, buttonIndex) => {
                      if (buttonIndex == 1) {
                        ...
                        focusNote.noteTitle = taskUtils.replaceStringStartWithSquarebracketContent(
                          focusNote.noteTitle,
                        ...
                        // taskUtils.sortNoteByVolNum(targetJournalNote, 1)
                        let bookLibraryNote = MNNote.new("49102A3D-7C64-42AD-864D-55EDA5EC3097");
                        MNUtil.undoGrouping(() => {
                          bookLibraryNote.addChild(focusNote.note);
                          focusNote.focusInMindMap(0.5);
  ...
  MNTaskGlobal.registerCustomAction("referenceAuthorNoteMake", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        focusNotes.forEach((focusNote) => {
          taskUtils.referenceAuthorNoteMake(focusNote);
        });
      } catch (error) {
        MNUtil.showHUD(error);
        MNUtil.copy(error);
  ...
  MNTaskGlobal.registerCustomAction("referenceBibInfoCopy", async function (context) {
    ...
    bibContentArr = [];
    focusNotes.forEach((focusNote) => {
      bibContentArr.push(taskUtils.extractBibFromReferenceNote(focusNote));
    });
    ...
    UIAlertView.showWithTitleMessageStyleCancelButtonTitleOtherButtonTitlesTapBlock(
      ...
      (alert, buttonIndex) => {
        try {
          MNUtil.undoGrouping(() => {
            userInput = alert.textFieldAtIndex(0).text;
            let keywordArr = taskUtils.splitStringByFourSeparators(userInput);
            ...
            if (buttonIndex === 1) {
              ...
              keywordArr.forEach((keyword) => {
                ...
                if (keywordIndexInFocusNote == -1) {
                  ...
                  let keywordContinuousLinksArr = taskUtils.getContinuousSequenceFromNum(
                    keywordLinksArr,
                    keywordTextIndex,
                  );
                  focusNote.moveComment(
      ...
      (alert, buttonIndex) => {
        try {
          MNUtil.undoGrouping(() => {
            ...
            if (buttonIndex === 1) {
              taskUtils.referenceInfoYear(focusNote, year);
            }
          });
        } catch (error) {
          MNUtil.showHUD(error);
          ...
          MNUtil.undoGrouping(() => {
            ...
            if (buttonIndex === 1) {
              ...
              try {
                MNUtil.undoGrouping(() => {
                  if (findKeyword) {
                    // MNUtil.showHUD(taskUtils.findCommonComments(targetKeywordNoteArr, "相关文献："))
                    let idsArr = taskUtils.findCommonComments(targetKeywordNoteArr, "相关文献：");
                    if (idsArr.length > 0) {
                      // 找到了共有的链接
                      ...
                      let resultNote;
                      let combinations = taskUtils.generateArrayCombinations(keywordArr, " + "); // 生成所有可能的组合
                      ...
                      try {
                        if (!findResultNote) {
                          ...
                          // 重新合并模板
                          taskUtils.cloneAndMerge(resultNote, "DE4455DB-5C55-49F8-8C83-68D6D958E586");
                        }
                        idsArr.forEach((id) => {
            ...
            if (buttonIndex === 1) {
              ...
              keywordArr.forEach((keyword) => {
                ...
                if (keywordIndexInFocusNote == -1) {
                  ...
                  let keywordContinuousLinksArr = taskUtils.getContinuousSequenceFromNum(keywordLinksArr, 0);
                  focusNote.moveComment(
                    focusNote.comments.length - 1,
                    keywordContinuousLinksArr[keywordContinuousLinksArr.length - 1] + 1,
                  );
      ...
      (alert, buttonIndex) => {
        try {
          MNUtil.undoGrouping(() => {
            ...
            let authorArr = taskUtils.splitStringByThreeSeparators(userInput);
            ...
            if (buttonIndex === 1) {
              ...
              authorArr.forEach((author) => {
                findAuthor = false;
                let possibleAuthorFormatArr = [...new Set(Object.values(taskUtils.getAbbreviationsOfName(author)))];
                ...
                if (authorIndexInFocusNote == -1) {
                  ...
                  let authorContinuousLinksArr = taskUtils.getContinuousSequenceFromNum(
                    authorLinksArr,
                ...
                if (focusNoteIndexInTargetAuthorNote == -1) {
                  ...
                  if (taskUtils.getReferenceNoteType(focusNote) == "book") {
  ...
  MNTaskGlobal.registerCustomAction("referenceInfoInputRef", async function (context) {
    ...
    UIAlertView.showWithTitleMessageStyleCancelButtonTitleOtherButtonTitlesTapBlock(
      ...
      (alert, buttonIndex) => {
        try {
          MNUtil.undoGrouping(() => {
            let referenceContent = taskUtils.extractRefContentFromReference(alert.textFieldAtIndex(0).text);
            referenceContent = taskUtils.formatEnglishStringPunctuationSpace(referenceContent);
            if (buttonIndex == 1) {
              let thoughtHtmlCommentIndex = focusNote.getCommentIndex("相关思考：", true);
          ...
          MNUtil.undoGrouping(() => {
            if (buttonIndex == 1) {
              if (focusNote.noteTitle !== "") {
                ...
                let referenceContent = taskUtils.extractRefContentFromReference(focusNote.excerptText);
                referenceContent = taskUtils.formatEnglishStringPunctuationSpace(referenceContent);
                let refNum = alert.textFieldAtIndex(0).text;
                if (refNum == 0) {
                  ...
                  currentDocmd5 = MNUtil.currentDocmd5;
                  let targetNoteId = taskConfig.referenceIds[currentDocmd5]
    ...
    try {
      MNUtil.undoGrouping(() => {
        if (focusNote.noteTitle !== "") {
          ...
          let referenceContent = taskUtils.extractRefContentFromReference(focusNote.excerptText);
          referenceContent = taskUtils.formatEnglishStringPunctuationSpace(referenceContent);
          let refNum = taskUtils.extractRefNumFromReference(focusNote.excerptText);
          if (refNum == 0) {
            MNUtil.showHUD("当前文档没有绑定卡片 ID");
          } else {
            currentDocmd5 = MNUtil.currentDocmd5;
  ...
  MNTaskGlobal.registerCustomAction("referenceMoveLastCommentToThought", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        taskUtils.referenceMoveLastCommentToThought(focusNote);
      });
    });
  });
  ...
  MNTaskGlobal.registerCustomAction("referenceMoveLastTwoCommentsToThought", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        taskUtils.referenceMoveLastTwoCommentsToThought(focusNote);
      });
    });
  });
  ...
  MNTaskGlobal.registerCustomAction("referenceAddThoughtPointAndMoveLastCommentToThought", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        focusNotes.forEach((focusNote) => {
          taskUtils.referenceAddThoughtPoint(focusNote);
          taskUtils.referenceMoveLastCommentToThought(focusNote);
        });
      });
    } catch (error) {
  ...
  MNTaskGlobal.registerCustomAction("referenceMoveUpThoughtPoints", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        focusNotes.forEach((focusNote) => {
          taskUtils.referenceMoveUpThoughtPoints(focusNote);
        });
      } catch (error) {
        MNUtil.showHUD(error);
      }
  ...
  MNTaskGlobal.registerCustomAction("moveLastCommentToProofStart", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        let targetIndex = taskUtils.getProofHtmlCommentIndex(focusNote) + 1;
  ...
  MNTaskGlobal.registerCustomAction("moveProofToStart", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        let targetIndex = taskUtils.getProofHtmlCommentIndex(focusNote) + 1;
        taskUtils.moveProofToIndex(focusNote, targetIndex);
      } catch (error) {
        MNUtil.showHUD(error);
      }
    });
  ...
  MNTaskGlobal.registerCustomAction("addProofFromClipboard", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        MNUtil.excuteCommand("EditPaste");
        MNUtil.delay(0.1).then(() => {
          taskUtils.moveLastCommentToProof(focusNote);
        });
      });
    } catch (error) {
      MNUtil.showHUD(error);
            ...
            if (buttonIndexI == 1) {
              for (let i = 0; i < focusNote.comments.length; i++) {
                ...
                if (
                  ...
                  comment.text.includes("方法" + taskUtils.numberToChinese(methodNum))
                ) {
                  methodIndex = i;
                  findMethod = true;
                }
                        ...
                        if (buttonIndex == 1) {
                          ...
                          focusNote.appendMarkdownComment(
                            '<span style="font-weight: bold; color: #014f9c; background-color: #ecf5fc; font-size: 1.15em; padding-top: 5px; padding-bottom: 5px"> 方法' +
                              taskUtils.numberToChinese(methodNum) +
                              "：" +
                              methodComment +
                              "</span>",
                            methodIndex,
            ...
            if (buttonIndex == 1) {
              ...
              focusNote.appendMarkdownComment(
                '<span style="font-weight: bold; color: #081F3C; background-color: #B9EDD0; font-size: 1.15em; padding-top: 5px; padding-bottom: 5px"> 反例' +
                  taskUtils.numberToChinese(antiexampleNum + 1) +
                  "：" +
                  antiexampleComment +
                  "</span>",
                targetIndex,
            ...
            if (buttonIndex == 1) {
              ...
              focusNote.appendMarkdownComment(
                '<span style="font-weight: bold; color: #081F3C; background-color: #B9EDD0; font-size: 1.15em; padding-top: 5px; padding-bottom: 5px"> 方法' +
                  taskUtils.numberToChinese(methodNum + 1) +
                  "：" +
                  methodComment +
                  "</span>",
                targetIndex,
  ...
  MNTaskGlobal.registerCustomAction("moveLastTwoCommentsToProof", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        taskUtils.moveLastTwoCommentsToProof(focusNote);
      });
    });
  });
  ...
  MNTaskGlobal.registerCustomAction("renewProof", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        taskUtils.renewProof(focusNotes);
      });
    } catch (error) {
      MNUtil.showHUD(error);
    }
          ...
          MNUtil.undoGrouping(() => {
            ...
            if (buttonIndex == 1) {
              ...
              if (!findMethod) {
                ...
                taskUtils.moveProofToMethod(focusNote, methodNum);
              }
            }
          });
        } catch (error) {
        ...
        try {
          if (focusNote.ifIndependentNote()) {
            ...
            if (taskConfig.windowState.preprocess) {
              focusNotes.forEach((focusNote) => {
                taskUtils.TemplateMakeNote(focusNote);
              });
            } else {
              UIAlertView.showWithTitleMessageStyleCancelButtonTitleOtherButtonTitlesTapBlock(
                "撤销制卡",
  ...
  MNTaskGlobal.registerCustomAction("TemplateMakeChildNotes", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNote.childNotes.forEach((childNote) => {
        taskUtils.TemplateMakeNote(childNote);
  ...
  MNTaskGlobal.registerCustomAction("convetHtmlToMarkdown", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        taskUtils.convetHtmlToMarkdown(focusNote);
      });
    } catch (error) {
      MNUtil.showHUD(error);
    }
  ...
  MNTaskGlobal.registerCustomAction("clearContentKeepMarkdownText", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        taskUtils.clearContentKeepMarkdownText(focusNote);
      });
    } catch (error) {
      MNUtil.showHUD(error);
    }
  ...
  MNTaskGlobal.registerCustomAction("moveLastTwoCommentsToThought", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        taskUtils.moveLastTwoCommentsToThought(focusNote);
      });
    });
  });
  ...
  MNTaskGlobal.registerCustomAction("moveUpLinkNotes", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        taskUtils.moveUpLinkNotes(focusNotes);
      });
    } catch (error) {
      MNUtil.showHUD(error);
    }
  ...
  MNTaskGlobal.registerCustomAction("clearAllFailedMN3Links", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        focusNotes.forEach((focusNote) => {
          taskUtils.linksConvertToMN4Type(focusNote);
          // 从最后往上删除，就不会出现前面删除后干扰后面的 index 的情况
          for (let i = focusNote.comments.length - 1; i >= 0; i--) {
            let comment = focusNote.comments[i];
  ...
  MNTaskGlobal.registerCustomAction("clearAllFailedLinks", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        focusNotes.forEach((focusNote) => {
          taskUtils.clearAllFailedLinks(focusNote);
        });
      } catch (error) {
        MNUtil.showHUD(error);
      }
    ...
    MNUtil.undoGrouping(() => {
      try {
        if (focusNotes.length == 1) {
          ...
          let idsArr = taskUtils.getNoteIdArr(focusNotes);
          MNUtil.copy(idsArr);
  ...
  MNTaskGlobal.registerCustomAction("copyFocusNotesURLArr", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        let idsArr = taskUtils.getNoteURLArr(focusNotes);
        MNUtil.copy(idsArr);
        MNUtil.showHUD(idsArr);
      } catch (error) {
        MNUtil.showHUD(error);
  ...
  MNTaskGlobal.registerCustomAction("changeChildNotesPrefix", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        taskUtils.changeChildNotesPrefix(focusNote);
        focusNote.descendantNodes.descendant.forEach((descendantNote) => {
          if ([0, 1, 4].includes(descendantNote.note.colorIndex)) {
            try {
              // MNUtil.undoGrouping(()=>{
              taskUtils.changeChildNotesPrefix(descendantNote);
              // })
            } catch (error) {
              MNUtil.showHUD(error);
            }
  ...
  MNTaskGlobal.registerCustomAction("pasteNoteAsChildNote", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        taskUtils.pasteNoteAsChildNote(focusNote);
      } catch (error) {
        MNUtil.showHUD(error);
      }
    });
    ...
    MNUtil.undoGrouping(() => {
      try {
        focusNotes.forEach((focusNote) => {
          ...
          taskUtils.linkRemoveDuplicatesAfterIndex(focusNote, applicationHtmlCommentIndex);
        });
      } catch (error) {
        MNUtil.showHUD(error);
      }
  ...
  MNTaskGlobal.registerCustomAction("renewAuthorNotes", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        ...
        taskUtils.cloneAndMerge(focusNote, "782A91F4-421E-456B-80E6-2B34D402911A");
  ...
  MNTaskGlobal.registerCustomAction("renewJournalNotes", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        ...
        focusNote.removeCommentByIndex(0);
        focusNote.removeCommentByIndex(0);
        taskUtils.cloneAndMerge(focusNote, "129EB4D6-D57A-4367-8087-5C89864D3595");
        focusNote.moveComment(focusNote.comments.length - 1, 0);
        focusNote.moveComment(focusNote.comments.length - 1, 0);
        focusNote.moveComment(focusNote.comments.length - 1, 0);
        focusNote.moveComment(focusNote.comments.length - 1, 0);
  ...
  MNTaskGlobal.registerCustomAction("renewPublisherNotes", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        ...
        focusNote.removeCommentByIndex(0);
        focusNote.removeCommentByIndex(0);
        taskUtils.cloneAndMerge(focusNote, "1E34F27B-DB2D-40BD-B0A3-9D47159E68E7");
        focusNote.moveComment(focusNote.comments.length - 1, 0);
        focusNote.moveComment(focusNote.comments.length - 1, 0);
        focusNote.moveComment(focusNote.comments.length - 1, 0);
        focusNote.moveComment(focusNote.comments.length - 1, 0);
  ...
  MNTaskGlobal.registerCustomAction("renewBookNotes", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        let title = focusNote.noteTitle;
        let yearMatch = taskUtils.isFourDigitNumber(taskUtils.getFirstKeywordFromTitle(title));
        if (yearMatch) {
          // MNUtil.showHUD(taskUtils.getFirstKeywordFromTitle(title))
          let year = taskUtils.getFirstKeywordFromTitle(title);
          taskUtils.referenceYear(focusNote, year);
  ...
  MNTaskGlobal.registerCustomAction("addThought", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        taskUtils.addThought(focusNotes);
      } catch (error) {
        MNUtil.showHUD(error);
      }
    });
  ...
  MNTaskGlobal.registerCustomAction("reappendAllLinksInNote", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      try {
        focusNotes.forEach((focusNote) => {
          taskUtils.reappendAllLinksInNote(focusNote);
        });
      } catch (error) {
        MNUtil.showHUD(error);
      }
  ...
  MNTaskGlobal.registerCustomAction("achieveCards", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        taskUtils.achieveCards(focusNote);
      });
    } catch (error) {
      MNUtil.showHUD(error);
    }
  ...
  MNTaskGlobal.registerCustomAction("renewCards", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        focusNotes.forEach((focusNote) => {
          // taskUtils.renewCards(focusNote)
          focusNote.renew();
        });
      });
    } catch (error) {
  ...
  MNTaskGlobal.registerCustomAction("renewChildNotesPrefix", async function (context) {
    ...
    try {
      MNUtil.undoGrouping(() => {
        taskUtils.renewChildNotesPrefix(focusNote);
      });
    } catch (error) {
      MNUtil.showHUD(error);
    }
  ...
  MNTaskGlobal.registerCustomAction("undoOKRNoteMake", async function (context) {
    ...
    MNUtil.undoGrouping(() => {
      focusNotes.forEach((focusNote) => {
        taskUtils.OKRNoteMake(focusNote, true);
      });
    });
  });

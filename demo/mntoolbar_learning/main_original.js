 ...
 *                           例如：/Users/xxx/Library/Containers/QReader.MarginNoteApp/Data/Library/MarginNote Extensions/MNToolbar
 *                           这个路径用于：
 *                           1. 加载插件资源（图片、配置文件等）
 *                           2. 存储插件数据
 *                           3. 定位插件文件
 * 
 * @returns {MNToolbarClass} 返回插件主类，供 MarginNote 管理
 */
JSB.newAddon = function (mainPath) {
  ...
  // MNUtils 是 MNToolbar 的依赖，提供了大量的 API
  ...
  var MNToolbarClass = JSB.defineClass(
    ...
    { /* Instance members - 实例方法 */
      ...
      sceneWillConnect: async function () { //Window initialize
        ...
        // getMNToolbarClass() 函数返回这个实例，供后续使用
        let self = getMNToolbarClass()
        
        // 🚀 步骤3：初始化插件核心功能
        // init() 方法会：
        // - 初始化工具类 (toolbarUtils)
        // - 初始化配置管理器 (toolbarConfig)
      ...
      onPopupMenuOnSelection: async function (sender) { // Clicking note
        ...
        if (!self.testController) {
          ...
          self.testController.dynamic = toolbarConfig.dynamic   // 动态模式配置
          self.testController.view.hidden = true                // 初始隐藏
          
          // 设置关联关系
          self.testController.addonController = self.addonController      // 关联主控制器
          self.addonController.dynamicToolbar = self.testController       // 反向关联
      ...
      onPopupMenuOnNote: async function (sender) { // Clicking note
        ...
        toolbarUtils.currentNoteId = sender.userInfo.note.noteId     // 全局也保存一份
        ...
        if (testController.view.hidden) {
          ...
          testController.setToolbarLayout()     // 设置工具栏布局
        }else{
          // 🔄 情况2：工具栏已经显示，只需要调整位置
      ...
      controllerWillLayoutSubviews: function (controller) {
            ...
            if (toolbarConfig.horizontal()) {
              ...
              self.addonController.setFrame(
                MNUtil.genFrame(
                  ...
                  currentFrame.y, 
                  40,  // 固定宽度
                  toolbarUtils.checkHeight(currentFrame.width, self.addonController.maxButtonNumber)  // 动态高度
                ),
                true  // 不触发动画
              )
            } else {
              ...
              self.addonController.setFrame(
                MNUtil.genFrame(
                  currentFrame.x, 
                  currentFrame.y, 
                  toolbarUtils.checkHeight(currentFrame.height, self.addonController.maxButtonNumber),  // 动态高度
                  40  // 固定宽度
                ),
                true  // 不触发动画
              )
        ...
        if (self.testController && !self.testController.view.hidden) {
          ...
          if (self.testController.onAnimate || self.testController.onResize) {
            ...
            let buttonNumber = toolbarConfig.getWindowState("dynamicButton");  // 获取动态工具栏的按钮数量
      ...
      queryAddonCommandStatus: function () {
        ...
        if (self.settingController) {
          let iCloudSync = toolbarConfig.iCloudSync  // 获取当前同步状态
        ...
        return {
            ...
            checked: toolbarConfig.dynamic  // 当前状态（用于显示勾选）
          };
      },
      ...
      onTextDidBeginEditing: function (param) {
        try {
          ...
          toolbarUtils.textView = textView  // 全局保存，其他地方可能需要
      ...
      openDocument: function (button) {
        ...
        MNUtil.postNotification("openInBrowser", {
          url: "https://mnaddon.craft.me/toolbar"
        })
      },
      ...
      try {
        ...
        // 📋 构建菜单项列表
        let selector = "toggleToolbarDirection:"  // 切换方向的方法名
        ...
        var commandTable = [
          ...
          // 🛠️ 主工具栏方向
          self.tableItem(
            '🛠️   Direction   ' + (toolbarConfig.vertical() ? '↕️' : '↔️'),  // 显示当前方向
            selector,
            "fixed"  // 参数：表示是固定工具栏
          ),
          ...
          self.tableItem(
            ...
            "toggleDynamic",
            undefined,
            toolbarConfig.dynamic  // 是否打勾（动态模式开启时打勾）
          ),
          
          // 🌟 动态工具栏方向
          self.tableItem(
            '🌟   Direction   ' + (toolbarConfig.vertical() ? '↕️' : '↔️'),
            selector,
            "dynamic"  // 参数：表示是动态工具栏
          ),
    ...
    { /* 静态方法 - 插件级别的生命周期方法 */
      ...
      addonWillDisconnect: async function () {
        ...
        let confirm = await MNUtil.confirm(
          "MN Toolbar: Remove all config?",    // 英文提示
          "MN Toolbar: 删除所有配置？"           // 中文提示
        )
        
        if (confirm) {
          // 🗑️ 用户确认，删除所有配置
          toolbarConfig.remove("MNToolbar_dynamic")      // 动态模式设置
          toolbarConfig.remove("MNToolbar_windowState")  // 窗口状态
          toolbarConfig.remove("MNToolbar_action")       // 按钮动作配置
          toolbarConfig.remove("MNToolbar_actionConfig") // 动作详细配置
        }
      },
  ...
  MNToolbarClass.prototype.ensureView = function (refresh = true) {
    try {
      ...
      // 1. 在当前窗口中
      // 2. 是学习视图的子视图
      if (toolbarUtils.isDescendantOfCurrentWindow(this.addonController.view) && 
   ...
   * ```javascript
   * let menuItems = [
   *   this.tableItem("打开工具栏", "toggleToolbar:", "", this.addonController.view.hidden),
   ...
   * @this {MNToolbarClass} - 插件主类实例
   * @returns {{title:string, object:MNToolbarClass, selector:string, param:any, checked:boolean}} 菜单项对象
   * 
   * @example
   * // 创建一个简单菜单项
   ...
   * 
   * // 创建一个带勾选的菜单项
   * let toggleItem = this.tableItem("显示工具栏", "toggleToolbar:", "", true)
   */

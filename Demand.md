> **Analyzing code comment task**
> The user requests to add detailed Chinese comments to the JavaScript code without modifying the original code. The commented-out line `// MNUtil.copyJSON(sender.userInfo)` seems to be a deprecated debug line, and the guidelines allow us to delete non-explanatory, deprecated comments. I think it's best to remove this line since it's not functional and doesn't add clarity. I'll ensure the JSDoc comment is added properly without altering the structure or indentation of code.> 

> **Preparing code with comments**
> The line `// MNUtil.copyJSON(sender.userInfo)` is commented out, likely due to being old debug code, so I'll remove it as instructed. I'll add a JSDoc comment using Chinese, giving an explanation of the function—focusing on the parameters and return values. I’ll also include brief line-level comments to explain logical checks like verifying if MNUtil is available and ensuring the window is in focus. The indentation and line endings must remain unchanged, except for the JSDoc comment above the function.
> *Thought for 12 seconds*

```javascript
/**
 * 设置视频播放时间点的回调函数  
 *
 * @param {Object} sender - 事件发送者对象，通常由系统或其他模块触发  
 * @param {Object} sender.userInfo - 事件附带的用户信息  
 * @param {number} sender.userInfo.time - 需要跳转的视频时间（秒）  
 * @returns {void}
 * @throws {Error} 当 MNUtil 未定义或当前窗口不在焦点时不会抛出异常，而是直接返回，避免后续逻辑执行
 */
onSetVideo: function (sender) {
    // 若工具库 MNUtil 尚未加载，则直接返回，防止调用其方法时报错
    if (typeof MNUtil === 'undefined') {
        return
    }
    // 只有当前窗口处于焦点时才继续执行，避免后台窗口误触发
    if (self.window !== self.appInstance.focusWindow) {
        return
    }
    // 通过注入脚本，将第一个 <video> 元素的播放位置跳转到指定时间
    self.addonController.runJavaScript('document.getElementsByTagName(\'video\')[0].currentTime = '+sender.userInfo.time)
}
```

[!] 性能提示：若页面包含多个 `<video>` 元素或元素动态创建，建议在调用前缓存或校验目标视频节点，以减少频繁的 DOM 查询和潜在的空指针问题。  
[!] 安全提醒：拼接字符串执行 `runJavaScript` 存在代码注入风险，应确保 `sender.userInfo.time` 已做类型与范围校验，防止恶意输入导致任意脚本执行。


上面是 AI 的回答，可以看到输出了 AI 的思考！导致我得手动删除！请继续优化 XDYY_PROMPTS.codeAnalysis，修复上面的问题



```javascript
/**
 * Initializes a set of static images for use in the application.
 * This method is designed to load multiple image assets based on a given main path.
 * 
 * @param {string} mainPath - The base directory path where the images are located.
 *                            This path is concatenated with specific filenames to generate
 *                            the full file paths for the images.
 * 
 * @example
 * MyClass.init('/assets/images');
 * // This will load images from '/assets/images/screen.png', '/assets/images/link.png', etc.
 * 
 * [!] 性能提示：如果mainPath包含大量的资源文件，可能会影响初始化时间，建议按需加载。
 * [!] 安全提醒：确保mainPath是可信路径，避免路径注入风险。
 */
static init(mainPath){
    // Store the base directory path for later use
    this.mainPath = mainPath;

    // Load the "screen.png" image and assign it to a static property
    this.screenImage = MNUtil.getImage(mainPath + `/screen.png`);
    // Load the "link.png" image, possibly used for hyperlinking or navigation icons
    this.linkImage = MNUtil.getImage(mainPath + `/link.png`);
    // Load the "home.png" image, typically used for a home button or icon
    this.homeImage = MNUtil.getImage(mainPath + `/home.png`);
    // Load the "goforward.png" image, representing a forward navigation button
    this.goforwardImage = MNUtil.getImage(mainPath + `/goforward.png`);
    // Load the "goback.png" image, representing a back navigation button
    this.gobackImage = MNUtil.getImage(mainPath + `/goback.png`);
    // Load the "reload.png" image, commonly used for a reload or refresh button
    this.reloadImage = MNUtil.getImage(mainPath + `/reload.png`);
    // Load the "stop.png" image, likely used to indicate a stop action
    this.stopImage = MNUtil.getImage(mainPath + `/stop.png`);
    // Load the "webapp.png" image, possibly representing a web application icon
    this.webappImage = MNUtil.getImage(mainPath + `/webapp.png`);
    // Load the "more.png" image with a scaling factor of 2.5
    // The second parameter (2.5) might specify a resizing or scaling factor for the image
    this.moreImage = MNUtil.getImage(mainPath + `/more.png`, 2.5);
}
```


上面是AI 返回的结果，请继续优化 XDYY_PROMPTS.codeAnalysis。满足：
1. 解析要用中文
2. 不需要 @example
3. 其余的你看是否需要优化 ultrathink


---
现在 ocrWithAI 功能的效果很不错，要开发一个新的。
主要痛点是，如果内容太长了，我可能不 OCR 了，而是我直接电脑上复制代码，然后我会粘贴成为 focusNote 的子卡片 A的第一条评论。所以增加一个直接通过选中 A 卡片触发功能，就是 A 的第一条评论作为处理的源代码，然后后续是一样的，成功处理了之后再删除卡片 A，A.removeFromParent() 可以删除卡片。ultrathink

---
static addHistory(type,detail){
    if (this.focusHistory.length>=10) {
        this.focusHistory.shift()
    }
    let history = {type:type,time:Date.now(),...detail}
    this.focusHistory.push(history)
    // MNUtil.copy(this.focusHistory)
}

的内容，最后怎么结果是
```javascript
/**
 * @param {number} a - 第一个数字参数
 * @param {number} b - 第二个数字参数
 * @returns {number} 两个数字的和
 * @throws {TypeError} 如果参数不是数字类型
 * @static
 */
function addHistory(a, b) {
  // 详细注释：此函数用于计算两个数字的加和并记录历史记录
  // 实现原理：直接使用JavaScript的加法运算符进行计算
  // 算法复杂度：O(1)，加法操作是常数时间复杂度
  // 为什么选择这种实现方式：简单直接，符合数学加法的基本定义
  // 关键代码行的作用说明：
  // - 使用加法运算符计算结果
  // - 检查参数类型确保输入有效
  
  if (typeof a !== 'number' || typeof b !== 'number') {
    throw new TypeError('Both parameters must be numbers');
  }
  
  const result = a + b;
  
  // 记录历史记录的逻辑（假设有记录历史的功能）
  // 这里省略具体实现，因为重点在于加法计算本身
  
  return result;
}
```

[!] 性能提示：如果此函数被频繁调用且参数类型固定，可以考虑使用类型守卫或缓存机制优化性能。
[!] 安全提醒：确保输入参数类型正确，避免潜在的类型错误导致程序崩溃。

---

代码完全错的！  请优化提示词！ultrathink

---
## 🤖 AI 代码分析

```javascript
/**
 * 添加历史记录到焦点历史数组中。
 * @param {string} type - 历史记录的类型。
 * @param {Object} detail - 历史记录的详细信息。
 * @static
 */
static addHistory(type, detail) {
  if (this.focusHistory.length >= 10) {
    this.focusHistory.shift();
  }

  let history = { type, time: Date.now(), ...detail };

  this.focusHistory.push(history);
}
```

[分步骤的详细解释]
1. **注释清理**：删除了非解释性注释，保留了授权注释和文档性注释。
2. **代码格式化**：使用了2空格缩进，调整了操作符和对象/数组周围的空格，并使用了分号。
3. **JSDoc生成**：添加了函数功能描述、参数说明、返回值说明和错误说明，并标记了静态方法。
4. **函数解释**：
   - 首先检查`focusHistory`数组的长度是否大于或等于10，如果是，则使用`shift()`方法移除数组的第一个元素。
   - 然后创建一个新的历史记录对象`history`，包含类型、当前时间戳和详细信息。
   - 最后将新的历史记录对象添加到`focusHistory`数组中。
5. **优化建议**：
   [!] `Date.now()`是一个轻量级函数，适合频繁调用，但如果有更高的性能要求，可以考虑缓存时间戳。

[优化建议]
[!] `Date.now()`是一个轻量级函数，适合频繁调用，但如果有更高的性能要求，可以考虑缓存时间戳。


---
上面是 AI 的回答，请基于下面的要求优化里面的提示词：
1. 不需要 `## 🤖 AI 代码分析`
2. AI 做了什么的总结不需要，比如
```
1. **注释清理**：删除了非解释性注释，保留了授权注释和文档性注释。
2. **代码格式化**：使用了2空格缩进，调整了操作符和对象/数组周围的空格，并使用了分号。
3. **JSDoc生成**：添加了函数功能描述、参数说明、返回值说明和错误说明，并标记了静态方法。
```
就不需要
3. [分步骤的详细解释] 这一块去掉吧，就保留建议。
4. 然后详细解释是用 JS 的代码注释写到 jS 代码里面！ultrathink

---
请学习 mntoolbar 的 ocrAsProofTitleWithTranslation 函数。我现在需要给 menu_codeLearning 按钮添加一个功能。
1. 先 OCR 卡片，这一步和 ocrAsProofTitleWithTranslation 函数一样。
2. 然后获取 OCR 的结果，再进行 AI 处理，下面是提示词：

````
你是一个专业的JavaScript代码处理引擎，我将提供类中的函数代码（static/prototype/实例方法）。请严格按以下步骤处理：

1. **注释清理**：
   - 删除所有非解释性注释（如`// 临时调试`，`/* 废弃代码 */`）
   - 保留授权注释（如`/*! MIT License */`）和文档性注释

2. **代码格式化**：
   ```javascript
   // 要求：
   - 2空格缩进
   - 操作符空格：`a+b` → `a + b`
   - 对象/数组空格：`{a:1}` → `{ a:1 }`
   - 分号使用：符合StandardJS规范
   ```

3. **JSDoc生成**：
   ```javascript
   /**
    * 函数功能描述
    * @param {Type} param - 参数说明
    * @returns {ReturnType} 返回值说明
    * @throws {ErrorType} 错误说明
    * @example // 使用示例
    */
   ```
   - 类型推断顺序：1) 代码分析 2) TypeScript类型 3) `any`
   - 必需标记：
     - `@static` 静态方法
     - `@memberof ClassName.prototype` 原型方法

4. **函数解释**：
   ```text
   // 按执行顺序解释：
   1. [逻辑块1]：说明目的（例：数据初始化）
   2. [逻辑块2]：核心算法说明（含复杂度O(n)）
   3. [边界处理]：特殊条件检查说明
   ```

5. **优化建议**（以`[!]`标记）：
   ```text
   [!] 性能提示：可改用Map提升查找效率
   [!] 安全提醒：缺少输入验证
   ```

**输出格式**：
````javascript
// [文件路径，如: src/utils/String.js]
[格式化后的完整代码]
```
// 函数解析
[分步骤的详细解释]
[优化建议]
````

3. 然后学习一下 manageCommentsByPopup 函数是如何移动内容到某个字段的。我需要把上面 AI 给的回答，添加到选中的卡片的评论中，并移动到“分析”字段。ultrathink





---
我会给AI 一个 JavaScript 里某个函数，主要是类里的函数，比如 static 的 或者是 prototype 的代码，然后我希望 AI 要实现以下几步
1. 如果是非解释性注释，就直接去掉。
2. 按照 JavaScript 正确的空格进行缩进，因为可能缺少缩进
3. 给函数添加 JSDoc
4. 尽可能详细地解释函数体里的实现，因为我在学习代码。
请你看看是否需要进一步优化，然后帮我写一个 AI 提示词。
---
请你为 mntoolbar 开发一个新按钮，按钮用 code.png，主要用来学习代码。名字就叫“代码学习”。先开发单击功能。功能不要写在注册的地方，而是 utils 里面进行封装。
我先说一下背景。我是要在 MarginNote4 里学习代码，我把代码转成了 PDF 在里面学习。mntoolbar 是 MarginNote 4 的插件。
卡片对应的具体的插件和文件，比如
```
🧩 mnutils
```
为标题的卡片，记为 A，这是代表着“根卡片”也就是对应的插件. A 有一些子卡片，其中一个卡片的标题为
```
mnutils.js
```
记为 B，这个就对应着 mnutils 插件中的 mnutils.js 文件。
然后 B 的子卡片显然就是文件里的具体内容了，我是用“class”来分的，比如
```
MNNote 类
```
记为 C，C 是 B 的子卡片，那么也就是我们得到了 A-B-C 的结构。对应了“插件名-文件名-类名”的结构。
然后 C 的子卡片就是主要的内容了，就是具体变量，函数等。
大致有这么几类
- 类的静态变量
- 类的静态方法
- 实例方法
- 实例 Getter 方法
- 实例 Setter 方法

具体单击后的效果是，给一个弹窗，然后选择上述集中类型，对应不同的处理。
首先基本差不多的就是要获取知识点卡片 D 的上面的卡片，也就是上方的 ABC，比如 D 是“new”为标题的卡片，对应 static new也就是静态方法。
那么最后要的效果是标题变成
```
【类：静态方法 >> mnutils/mnutils.js/MNNote 类】; MNNote.new; this.new
```
不管是什么类型的，“mnutils/mnutils.js/MNNote 类” 这个部分的获取逻辑基本是一致的，就是获取父卡片对应 C，C 的父卡片对应 B，B 的父卡片对应 A（不过用正则表达式看文件名是否符合要求，比如插件有 emoji，文件名有后缀，类的有“类”字）

然后就是 “>>” 前面的内容

- 类的静态变量：`类：静态变量`
- 类的静态方法：`类：静态方法`
- 实例方法：`实例方法`
- 实例 Getter 方法：`实例：Getter 方法`
- 实例 Setter 方法：`实例：Setter 方法`

最后是 “new” 要变成几种变体，主要就是几种调用方式，比如静态方法就是通过 this 或者类名调用，其余同理。最后用分号连接。

目前已经开发了一些了，你看看是否需要完善
ultrathink



---
优化一下 mnutils 的 manageCommentsByPopup 函数的评论多选功能。现在有一个痛点是如果评论太多了的话，我一个个点非常费劲，有的时候可能要点十几条，每一条都要等一次新的弹窗，十分的麻烦，所以我想就是多选的里面加一个选择起始和终止选项的功能，也就是我只要确定两个端点，中间的就默认选中。ultrathink



---
优化一下 mnutils 的 manageCommentsByPopup 函数的删除评论功能。增加一个功能：
1. 如果选中的只有一条评论，并且这个评论是一个卡片的链接
2. 点击了删除评论
3. 那么此时弹窗除了确认是否删除之外，还要增加一个“确认并复制行内链接”的选项。如果点击之后就类似于 mntoolbar 的 copyMarkdownVersionFocusNoteURL 动作，弹出一个框让用户填写引用词，确认后可以复制一个 Markdown 的链接。 ultrathink


----
类似于 addNewSummaryNote 和 addNewIdeaNote 功能，现在需要增加一个新增反例卡片的功能。稍微区别一下的是
此时的 HtmlMarkdown 评论用 alert 类型。选中卡片 A 增加反例卡片 B 的话，A 到 B 的链接移动到卡片最上方。ultrathink
---
优化一下 mnutils 的 manageCommentsByPopup 函数的提取评论功能。
目前提取之后可以选择删除选中的评论。我希望你先看看提取出来的那个部分，我们也有一个链接处理。
比如 A 卡片我们选择了一些内容记为 content，提取为 B，我要检测 content 里链接对应的卡片的最后一个字段里是否包含 A 的链接，如果有的话，就删掉这个链接。

关于 B 的处理你要一起考虑。因为 B 的那个链接和 A 有关。

也就是如果我们不选择删除的话，那么就是当前的这个，只处理 B 的链接。
但如果选择了删除的话，那么处理完 B 的链接后，要把对应卡片里最后一个字段的 A 的链接删除 ultrathink
---
优化一下 mnutils 的 makeCard 函数。
最后增加一个处理：移动卡片里的所有总结链接到卡片最上方。
提示：用 MNComment 来判断类型，有一个类型是 Summary 有关的。 ultrathink
---
优化一下 mnutils 的showDefinitionCatalog 函数。
目前选中某个定义时，会弹出下方所有的归类卡片的信息，我希望增加一个选项是这个选中的定义本身，也就是点击后可以跳转到这个定义卡片。 ultrathink
---
mntoolbar 的 menu_proof 菜单里面有一些翻译的功能，但现在翻译的效果不太理想，因为我是一名数学系的学生，我希望你把翻译的提示词优化一下，让翻译的时候参考数学的专用词汇。 ultrathink
---
开发 mntoolbar 。在 menu_comment 的长按菜单里增加新功能：实现最后一条评论的快速移动。
【背景】目前 menu_comment 的单击功能非常强大，很自由地处理评论，但是问题就是太自由了，很多时候我要移动最后一条评论的时候需要太多步了
【需求】对于最后一条评论，可以对于常见的移动位置，直接一个位置写一个功能，比如移动到“所属”区（这个默认就是移动到最下方）
【一些想法】菜单的条件中可以直接使用字符串来作为一种“划分”，所以可以
```
menuItem = [
  ...
  "移动最后一条评论",
  {
    action: "...",
    title: "    移动到所属区",
  }
  ...
]
```
"    移动到所属区" 加空格的话，视觉上就能产生是一个子菜单的效果，这样就能实现快速移动最后一条评论到常用的位置。

然后所属区的话，只有归类卡片的模板里才有这个字段，如果选中的卡片没有这个字段的话，那就什么也不操作就行 ultrathink

---
优化 mnutils 里的 MNMath 类 的 addTemplate 函数。
现在主要有三种增加归类模板的方式。往下层增加，往上层增加，增加兄弟关系的归类卡片。
现在希望你能完善第三种，也就是增加兄弟关系的归类卡片。
1. 现在往下层增加的方式支持通过“//”来划分，实现多个归类卡片的增加，所以你先了解这个实现原理
2. 然后我希望增加兄弟关系的归类卡片的方式也能通过“//”来划分，增加多个归类卡片。
也就是比如
```
A - B
 |- C
```
这样来表示 B 和 C 是 A 的子卡片，B 和 C 是兄弟关系。

如果我点击 B，调用 addTemplate 函数，输入框里输入
```
测试//一下
```
那么会生成
```
A - B
 |- C
 |- D - E
```
其中 D 的标题的内容部分（也就是引号里面的内容）是`测试`, E 的标题的内容部分是`测试一下`。 ultrathink
---
优化 mnutils 里的 MNMath 类 的 manageCommentsByPopup 函数。
现在提取出来有一个问题是如果提取出来的评论里面有链接的话，现在有进一步的需求：
  - 假设我们原来的卡片是A，提取出来的是 B
  - 如果 B 的评论里有链接到 C，那么 A 既然是 B 的原型，那么 A 里面肯定通过弹窗选的某个位置里也有 C 的链接，那么这个时候你需要看 C 这个卡片里有没有 A 的链接，如果有的话，就把 B 单向链接到 C，然后移动到 C 卡片里 A 链接的下面。
  - 我来说明一下这个场景，帮助你理解：一个命题卡片A，里面的证明可能会抽离出来变成一个单独的命题来使用，这个就是我们要的 B，但是证明里通过与 C 的双向链接来表明一些使用的关系，但是只克隆的话，B 里面就丢失了 C 到 B 这个命题的链接，但是从使用上看，B 本质上只是 A 的一部分，所以这个关系肯定是要继承的。
  你之前的修改有问题！你的方向反了，不是 C 到B，而是 B 到C ! ultrathink
---
修复一下  showSearchDialog 函数的 bug：最新的修改见 git fb443430d6f9f581c3c5044b482e00a74718ff38。
但我输入后点击添加,然后输入内容确定后,弹窗报错: MN Utils Error (showSearchDialog): TypeError. Attempted to assign to readonly property. ultrathink

---
优化 mnutils 里的 MNMath 类 的 manageCommentsByPopup 函数。
1. 现在的原理是克隆当前卡片，然后对克隆的进行操作，但是有一个问题，是克隆的这个 api 会把他的子卡片一起克隆了，所以最后你需要把克隆出来的卡片的子卡片删掉（如果有的话）删除卡片的 API 要在 mnutils.js 里找一下
2. 现在提取出来有一个问题是如果提取出来的评论里面有链接的话，现在有进一步的需求：
  - 假设我们原来的卡片是A，提取出来的是 B
  - 如果 B 的评论里有链接到 C，那么 A 既然是 B 的原型，那么 A 里面肯定通过弹窗选的某个位置里也有 C 的链接，那么这个时候你需要看 C 这个卡片里有没有 A 的链接，如果有的话，就把 B 单向链接到 C，然后移动到 C 卡片里 A 链接的下面。
  - 我来说明一下这个场景，帮助你理解：一个命题卡片A，里面的证明可能会抽离出来变成一个单独的命题来使用，这个就是我们要的 B，但是证明里通过与 C 的双向链接来表明一些使用的关系，但是只克隆的话，B 里面就丢失了 C 到 B 这个命题的链接，但是从使用上看，B 本质上只是 A 的一部分，所以这个关系肯定是要继承的。 ultrathink

---
请修复 mntoolbar 的 Bug。描述如下：
1. 点击 MarginNote 插件栏的 mntoolbar 插件会弹出一个菜单，里面除了 `setting` 外有`预处理模式`和`粗读模式`，但是现在点击没有反应。
2. 点击 MarginNote 插件栏的 mntoolbar 插件会弹出一个菜单，如果开启 `dynamic` 的话，点击卡片时会弹出一排按钮。这个按钮栏有一个短条按钮，点击也可以弹出类似点击 MarginNote 插件栏的 mntoolbar 插件弹出的菜单，那个里面也有 `预处理模式`和`粗读模式`，这两个点击可以弹窗显示切换，也就是切换功能是正常的，但是这个菜单里始终没有显示当前状态，本来如果开启的话，旁边会有一个✓。

在 github 里开 issue 记录这个问题，然后开始修复 ultrathink

---
开发 mntoolbar 的 menu_excerpt 的功能。
增加一个放在长按菜单里的功能：卡片 A 如果有一个子卡片 B，点击 B 触发这个功能，就通过“合并到子卡片”这个功能（你直接搜，能搜到对应的动作，然后你看一下原理是什么），B 合并到 A 后然后 A 把最新的评论内容移动到摘录区（如何移动到摘录区可以参考 manageCommentsByPopup 函数里的部分实现）。ultrathink
--
继续开发 MNMath 的搜索功能 showSearchDialog。
1. 如果只有一个搜索结果的话，定位卡片的时候，就不要定位结果卡片，而是直接定位这唯一的结果。
2. 搜索弹窗增加一个新的 bool 值：是否搜索关键词字段。我来解释一下
  - 你看 MNMath 的 types 可以发现有一些类型里面有“关键词”字段
  - 这种卡片一般会作为标题的补充，增加一些同义的内容到“关键词”字段里，方便搜索。
  - 但是要注意这个字段和其它字段不同，其它字段的内容和字段本身不属于同一条评论，而关键词字段的内容是直接写在这个字段里，比如“关键词： 泛函分析 线性代数”，这个要特别注意。由于字段本身是 HtmlComment 类型的，所以找到这个类型，且 text 属性是关键词开头的评论，然后正则表达式获取内容，就可以获取到这个卡片的“关键词”内容
  - 但要注意关键词内容可能是空的，如果是空的，就不管，或者一些卡片可能没有这个字段，那么如果“在关键词字段中搜索”的 bool 是 true 的话，没有这个字段的卡片依旧在标题里的搜索。但如果有关键词字段的话，就不仅在标题里搜索，也要看关键词字段的内容里有没有需要搜索的内容。 ultrathink
---
继续开发 MNUtils 的 MNMath 类。makeCard 函数进行知识卡片制卡时需要检测最后一个字段下方链接是否有任务卡片的链接，有的话就移动到 “相关链接”下方。
参考：什么是任务卡片。参考 MNTask 项目的 isTaskCard 函数，本质上就是判断链接对应卡片的标题。ultrathink
---
继续开发 MNUtils 的 MNMath 类。manageCommentsByPopup 函数优化一下：现在的弹窗目前都不支持返回上一个弹窗，如果选错了就得重来，所以涉及到的每一个弹窗都加一个返回上一层的选项 ultrathink
---
优化一下 moveUpThoughtPointsToBottom 动作。目前有一个很大的问题，就是如果 A 和 B 是命题卡片，A 和 B 进行了双向链接。
A 的评论的最后一条是 B 的链接，B 的最后一条是 A 的链接，这个时候才会先同时增加一个“- ” 到“相关思考”字段，然后再移动最后一条评论。
但现在的效果是只要有双链，比如 A 的最后一条是 B 的链接，但 B 的最后一条不是 A 的链接，这个也会触发！这很糟糕！所以要么就单独在 MNMath 类里封装一个函数，然后动作这里只是简单调用 ultrathink

---
很棒，我又想到，可以再加一个是可以限制类型的。
1. 主弹窗里增加一个选项，为“选择类型”
2. 点击后进入新的弹窗，里面可以多选，类型主要是：定义，命题，反例，思想方法，思路，问题。先加这几个，后面需要再加。
3. 然后这个和临时修改根目录一样是一次性的，不需要保存到iCloud。用户不点击“选择类型”时默认全选，但如果用户点击“选择类型”时弹窗里的几个类型全都默认取消，因为这个时候一般用户只需要某个特定类型，所以这样会方便。
4. 选择类型后再次弹出主弹窗就行。然后选择的类型在主弹窗上面进行文字说明。如果没有点击“选择类型”就默认“全部”
ultrathink
---
mntoolbar 的按钮的长按功能出现了映射错位问题，我怀疑是和按钮的那个 “custom” 与官方的按钮冲突了？请你仔细检查。
具体错位情况如下：
1. menu_card 长按是提示没有长按菜单
2. menu_proof 的长按变成了 menu_htmlmdcomment 的长按菜单
3. menu_htmlmdcomment 的长按变成了 menu_card 的长按菜单
---
类似于“搜索归类卡片”的 bool 开关，再增加一个是对于非归类卡片，是否搜索进行判定时不考虑前缀的开关。
1. 用 parseNoteTitle 可以获取无前缀的部分。
2. 举例：`【定义：xxx】yyy`，`yyy` 就是无前缀的部分。
3. 这样做是为了防止前缀的干扰，提高搜索效率和准确性。ultrathink
---
1. 刚刚的两个问题都没解决，我是触发了那个搜索按钮的单击动作
2. toolbar 的动作里不要写太多内容！函数应该封装在 MNMath 类里面！你这样分开不好排查错误！我怀疑和这个有关
ultrathink
---
还有一个最大的问题，返回的结果卡片是：
```
搜索：H∞(𝕋) AND L∞(𝕋)

📑 搜索结果：H∞(𝕋) AND L∞(𝕋)

📁 根目录：未知

⏰ 搜索时间：2025/7/16 10:42:35

📊 结果数量：108 个
```

但是没有任何链接！这个只返回数量没有任何用啊！而且我说了返回的结果卡片作为“37F2105C-35E4-4840-AD79-DA4702C36BE1”为 ID 的卡片的子卡片 ultrathink
---
如果我点击临时切换根目录之后，弹出的弹窗上方的“上千根目录”后是“未知”，应该是我选中的这张卡片的标题
---
我们刚刚增加了一个搜索的按钮，现在我们重点在 MNMath 里结合开发一下搜索的功能，放在 mntoolbar 的按钮里。
1. 搜索范围：我们的搜索只在知识库的子孙卡片内容，知识库的根目录的卡片 ID 为 B2A5D567-909C-44E8-BC08-B1532D3D0AA1。
2. 刚刚的 showDefinitionCatalog 函数变成放在长按菜单里，单击触发一般的搜索，也就是马上要开发的这个功能。
3. 默认只通过标题来搜索，因为卡片数量比较多，一般不深度搜索，后续再继续开发。
4. 具体的交互就是点击了弹出一个输入框给用户，用户可以通过`//` 来划分多个关键词，然后默认的搜索模式是多个关键词同时出现的卡片，也就是多个关键词的交集。
- 如果只有一个关键词的话，就是标题中完整包含了输入内容的标题，包括空格，比如 `a b` 就不能搜出 `ab`
- 多个关键词的话，每个关键词也是和上面说的一样，要严格对应，比如 `Bill-xkw` 那就必须是包含 `Bill-xkw` 的标题，不能是 `Bill xkw` 或者 `xkw Bill` 这样的。
5. 增加一个选项是“下一个词”，这个时候采取的方式和 4 中不一样，相当于用户是部分输入（注意也可能有 `//`，也要注意处理这个）点击“下一个词”后再重复弹出相同的弹窗让用户输入，直到用户点击“开始搜索”选项才开始搜索
6. 支持选择不同的“根目录”，这个涉及到一个配置的问题，所以你还需要看配置如何保存到 icloud
  - 默认是1 中的根目录。用户可以在输入框输入一个卡片 ID 或者 URL，弹窗里有一个选项是添加根目录，如果点击这个，就再弹一个输入框来记录这个根目录的标识语，下次切换根目录时就显示这个标识语。添加完之后再重新弹出主界面让用户输入或者切换根目录
  - 上面说了如何添加，然后如何切换？在弹窗里增加一个选项是“切换根目录”，然后就把包括 1 中的根目录的所有标识语以弹窗的方式显示，来让用户选择，并且在弹窗的时候就要有一个 emoji 来表示当前是哪一个根目录，否则用户不知道当前是哪个根目录。选择后就切换根目录，然后再重新弹出来让用户选择输入或是取消
    - 切换根目录中，除了1 中的特殊选项，还有一个特殊选项是以当前选中的卡片为根目录，但要注意，这是一次性的，也就是这个根目录只在这一次搜索中切换。选完就弹出弹窗让用户输入内容来搜索。

用户确认开始搜索后，就以当前的根目录为父卡片，去子孙卡片里搜索。

如何呈现搜索结果？目前的处理是：37F2105C-35E4-4840-AD79-DA4702C36BE1 为搜索筛选的看板，搜索一次就生成一张结果卡片，作为这个看板的子卡片。
结果卡片的话就是把搜索结果通过评论的方式添加到结果卡片的评论里，格式如下：
```
[待定的 emoji] 归类卡片
……
[待定的 emoji] 定义卡片
……
[待定的 emoji] 命题卡片
……
```
这个分隔的字段稍微加点样式，比如粗体，或者加个边框之类的，具体你看怎么处理。
然后这个类型就是通过 parseNoteTitle 函数来获取的，如果在这以外的情况，但也包含关键词的，就是“其它”。没有的类型就不写。
“……”的内容就是链接，但注意只需要单向的。比如搜索结果卡片是 A，其中一个结果是搜到了  B，那么只需要 B 到 A 链接即可，A 到 B 的链接不需要。

ultrathink

---
我刚想了一下，showDefinitionCatalog 功能如果只是一张张找的话，可能还是比较耗时，要么就一次找多张，比如先找了 A，然后根据 A 找 B，这样下去，先找 3-5 张，然后都不是的话，还可以继续往上再找一批。
ultrathink

---
现在继续开发 mnutils 的 MNMath 类，现在需要开发新功能：
【背景】目前的使用是定义类卡片通过 addTemplate 函数不断增加归类卡片，然后命题，反例等卡片都是加在归类卡片下方。但是随着卡片越来越多，现在要找到我们要的卡片越来越困难。所以我们需要加强我们的搜索能力。
【需要的新功能：找到定义类卡片的“目录”】
1. 能找到当前卡片（记为 focusNote）往上一直找父卡片，直到找到第一个定义类卡片 A，作为我们的目标，然后通过弹窗让我们确认是否要这张，除了不找了（也就是取消），我们还可以基于这个A 再像前面一样往前找定义类卡片B，直到通过卡片的标题让用户最终确认是这个卡片，我们把最后找到的这个定义类卡片记为 def。
2. 定义类卡片的一般结构如下：
```
【定义：前缀】标题
<!-- 下面是评论 -->
相关思考
……
包含
……
```
包含的下方是我们重点要处理的，这个需要写一个函数来处理。“包含”字段下方都是归类卡片的链接，我们第一步要用函数处理的就是把他们重新排序。我来具体解释一下这个需求：
你读一下 addTemplate 函数先，然后就知道归类卡片的标题结构是
```
“xxx”相关yy
```
其中 yy 可能是命题，反例等，定义卡片下方链接对应的归类卡片的标题类型可能是乱的，比如
```
“xxx”相关命题
“xxx”相关反例
“xxx”相关命题
“xxx”相关命题
```
所以你需要将这些链接按照归类卡片对应的类型（如何获取的话：你看 parseNoteTitle 函数），“按组”分类，比如先是命题的，然后是反例的，以此类推。

这个处理功能单独写一个函数，可能会单独使用

3. def 卡片先按 2 的方式进行检测和处理，然后我们读取 def 的“包含”下方的链接，通过 def.MNComments 获取到这些链接对应的卡片
  - 获取标题，然后把标题通过弹窗的方式显示出来
  - 用户点击弹窗的某个选项后，效果是在 FloatMindMap 里定位到对应的卡片
  - 如果用户点击取消，则不进行任何操作

综上所述：我们实现了能快速查找和解析定义类卡片的目录，并且能快速定位到对应的归类卡片，这样的话即使归类卡片很多的时候，我也能快速进行跳转。ultrathink


---
刚刚 mntoolbar 我更新到了官方的最新版本，具体请看 git 的最新两个 commit。
这次改动比较大，所以请你完整阅读分析现在的代码，然后
1. 看看我们通过 xdyy*.js 文件来实现按钮功能拓展功能，现在是否有问题？
2. “粗读模式”以及OCR 的模型配置都没有在 python的 update.py 里处理，所以请你把这些配置都加上。 ultrathink

----
现在继续开发 mnutils 的 MNMath 类的 manageCommentsByPopup 函数。
1. switch (buttonIndex) 是通过对应按钮的数字，但是这样有一个不足就是，如果把上面的数组里的元素换一下顺序，那么下面的 switch 语句必须整体都要改，很麻烦，请你优化。
2. 目前弹窗里的多选功能支持移动和删除，我希望你再增加一个：“提取”。从效果上来看，就是把选中的内容提取出来变成新的卡片，成为子卡片，不需要有标题。具体的实现方式如下：
- 假设选中的是 note，那么克隆一下 note 为 clonedNote，并且把 clonedNote 添加为 note 的子卡片
- 因为是克隆的，所以两个卡片的评论是一模一样的，对应的 index 也是一样的，所以要达到提取的效果，其实就是把 clonedNote 里在弹窗里选中的几个评论之外的评论都删除掉就行了。
ultrathink


---
ocrAsProofTitleWithTranslation 目前无法实现翻译功能，翻译的那个弹窗点击后仍然是直接把翻译前的内容作为了标题。请深入学习 mnautostyle 插件项目，这里开发了 autoAi* 的功能，其中有一个是自动翻译！刚好是我们要的，请学习他的实现方式，并迁移过来。ultrathink


---
我补充一下，使用 mntoolbar 的时候，默认是 MNUtils 的 API Key 填写好了的
---
1. 没有 MNUtil.alert 函数 
2. 你这个 API 配置很有问题,很麻烦. 并不需要, MNUtils 插件里有一个 Subscription Manager，我们是在这里有一个总的 API Key 填写，然后 AI 调用的话，只需要切换模型即可，什么都不需要配置。请你把这个操作学习过来，请再仔细学习 mnai 项目！ultrathink


---
现在开发 mntoolbar/mntoolbar 项目：
1. 现在已经有 ocrAsProofTitle 函数了，这个函数的作用是调用 OCR 对卡片进行 OCR 并将返回的结果设置为标题。
2. 现在我希望你仔细研究 mnai 和 mntoolbar 项目的代码，我希望实现的功能是：能够进一步调用 AI 来将上面返回的结果翻译为中文。所以你得非常清楚 mnai 插件是如何实现的 AI 调用的，以及返回的结果如何获取，以及对应 的 Prompt 如何设置，还有可以设置一些 Function call 这个可以作为参考，我们作为底层代码层，大概率用不到这个，我们主要是需要 AI 返回的结果。ultrathink

---
很好，现在我的需求是我会点击一张卡片，然后我希望对这个卡片的所有没有标题的子孙卡片都进行这样的 OCR 操作，你看看能不能行？ultrathink
---
继续开发 mntoolbar：
尝试在“证明”按钮的长按菜单里加一个功能：
将选中的卡片 OCR，并且把 OCR 的结果设置为标题。
1. 参考 toolbar 本来就有的 OCR 动作
2. 要注意 onFinish 的处理，因为 OCR 需要时间
3. 调用的模型和源和 toolbar 保持一致，默认用第一个，然后用 JSDoc 写其余的，然后我来选 
ultrathink
---
修复 mntoolbar 的 upwardMergeWithStyledComments ：在 git 的 e0e40164b75bb9353b6bb015cf22399c7bfacccc 和5a616902f5f20d4d42c758845c5ce18d30ada447 两次 commit 中删除了大量的 API。不知道是不是这次删除导致的问题。
请你先分析这个动作的逻辑和功能，然后目前的效果是除了第一张子卡片的标题变成了 HtmlMardown 评论添加，然后后续的合并之类的以及后续的卡片都没有变！请你找出问题并修复！ultrathink
---
现在开发 mnutils 的  HtmlMarkdownUtils：
增加一个新的类型叫 check，具体的样式为
```
<span id="check" style="font-weight:600;color:#34A853;background:#E6F7EE;border:2px solid #34A853;border-radius:4px;padding:4px 8px;display:inline-block;box-shadow:0 1px 2px rgba(52,168,83,0.2);margin:0 2px;line-height:1.3;vertical-align:baseline;position:relative;">🔍 CHECK
</span>
```
样式为上面的，emoji 也是上面的
默认的前缀是 CHECK。要注意兼容到 mntoolbar 的动作里面

然后 mntoolbar 新增一个按钮，内容为证明按钮,菜单为 menu_proof，单击动作为增加一个上面新增的这个 CHECK 的HtmlMarkdown 评论，不加任何内容。长按菜单的第一个就是制卡按钮里的有一个合并证明要点，这里也加一个一样的 ultrathink
---
1. JavaScript 基础的部分整体还是写的太生硬了，逻辑性不强，对新手不友好。
2. 学习 mntoolbar/mntoolbar_learning/main.js, mntoolbar/mntoolbar_learning/webviewController.js, mntoolbar/mntoolbar_learning/settingController.js 这三个文件中的 JSDoc 注释，里面详细地补充了一些对插件，尤其是 toolbar 插件里结构的框架，尽可能地把 JSDoc 注释里的内容充分学习凝练，然后写到 A_first_course_to_the_development_of_MN_Addon.md
3. 最后你要以一个完全新手的视角重新自己从头再读一遍这个文档，哪里觉得难懂的，哪里觉得不通俗的，哪里觉得不够详细的，哪里觉得不够深入浅出的，都要进行修改和补充。
4. `note.appendHtmlComment("<b>HTML</b>", "显示文本", 16);` 你这写的是错的，请读 mnai 项目学习如何正确使用 appendHtmlComment，如果你是从哪里看到的，说明那个地方也写错了，把那个地方一起改了。下面出现的 appendHtmlComment 的都要检查！
5. 
```// 通用函数
function changeNotesColor(context, colorIndex, colorName) {
  const { focusNotes } = context;
  
  if (!focusNotes || focusNotes.length === 0) {
    MNUtil.showHUD("❌ 请先选择笔记");
    return;
  }
  
  MNUtil.undoGrouping(() => {
    focusNotes.forEach(note => {
      note.colorIndex = colorIndex;
    });
    MNUtil.showHUD(`✅ 已改为${colorName}`);
  });
}
```
其中`const { focusNotes } = context;` 是不是有问题啊，是不是还得写成 `const { button, des, focusNote, focusNotes, self } = context;`，下面有好多都是！不知道你从哪里看到的 `const { focusNotes } = context;`  的写法，这是错误的，把原出处也要改掉！

ultrathink
---
请基于当前目录下的 mnai, mntask, mnutils, mntoolbar 项目，完善  ./A_first_course_to_the_development_of_MN_Addon.md, 我把里面初步的框架写好了。这是一个面向一个没有 JavaScript 基础的 MarginNote 插件开发新手的教学文档，所以尽可能写得通俗易懂，深入浅出。

1. 把几个项目里的所有 Markdown 文件全都精读一遍！包括每个项目里的 CLAUDE.md 文件，这些都是开发插件中的精华所在。
2. 然后精读 mnutils, mntask 和 mntoolbar 项目代码，这是开发的重点，尤其是 mnutils. 也是后续初学者开发主要学习的插件。
3. 然后再精读 mnai 项目代码，了解插件更多的处理可能性以及一些结合插件实现的操作，了解插件还能实现什么效果
4. 你要完成上面几个内容再开始写。JavaScript 基础不需要太深，但是
   - 在开发新按钮功能中涉及到的
   - MNUtils 的基础 API 中涉及到的
  这些要写详细
1. MNUtils 一定要总结最常见，最常用的 API，尤其是 MNNote 类和 MNUtil 类的 API，这些是开发插件的基础。

希望最终的效果是只需要通过你的这一个文档就能让一个没有 JavaScript 基础的初学者，能够快速入门 MarginNote 插件开发，并且能够独立开发出自己的插件。ultrathink

---
还是有大问题！很多函数还是被删了！比如 mergIntoAndRenewReplaceholder，对照你写的 deleted_mnnote_methods_report.md 一个一个排查！ mntoolbar 里只要出现的都不能删！！！！！！！ ultrathink
---
重点排查一下 git 的：5a616902f5f20d4d42c758845c5ce18d30ada447 里的更新内容。具体说明如下：

上一次我让你根据：
```
请你精简 mnutils/xdyyutils.js 文件里的代码，不需要处理 MNMath 和 HtmlMarkdownUtils 类和 Pangu 类。其余的都需要处理。

精简依据，也就是下面情况的要删除
1. 没有在 mntask 里用到的
2. 没有在 mntoolbar 里用到的

尤其是 xdyyutils.js 里面有很多 MNNote 和 MNUtil 类的函数补充，但是很多内容都已经在 MNMath 类里实现了，所以不需要了，但是还是有一部分在 mntoolbar 里有用到，这些暂时保留先。
```
简化了代码，但是我发现有一个具体的情况：你把很多 mntoolbar, MNMath, HtmlMarkdownUtils 里涉及到的函数也删除了！比如 MNNote 的扩展：moveCommentsByIndexArr （好像是这个）！

请你完整自查，看看是不是误删了什么！不要删多了！ultrathink
---
请你精简 mnutils/xdyyutils.js 文件里的代码，不需要处理 MNMath 和 HtmlMarkdownUtils 类和 Pangu 类。其余的都需要处理。

精简依据，也就是下面情况的要删除
1. 没有在 mntask 里用到的
2. 没有在 mntoolbar 里用到的

尤其是 xdyyutils.js 里面有很多 MNNote 和 MNUtil 类的函数补充，但是很多内容都已经在 MNMath 类里实现了，所以不需要了，但是还是有一部分在 mntoolbar 里有用到，这些暂时保留先。

注意：最后再精简 String 类部分的代码，因为这个有些函数用在了需要删除的函数里，但是你因为这个而没有删除，为了避免这个，所以最后处理。 ultrathink
---
我刚更新了 mntoolbar 的按钮菜单（xdyy_menu_registry.js 文件中）
这个是最新的版本，请你基于这个菜单，查看 xdyy_custom_actions_registry.js 文件里的动作，没有在菜单里的动作全都删除。
然后查看按钮 xdyy_button_registry.js 文件，删除菜单不在菜单文件里的按钮。
ultrathink


---
优化一下 convertFieldContentToHtmlMDByPopup 函数，参考 manageCommentsByPopup 函数的处理，支持多选就行。ultrathink

---
继续开发 MNMath 的 smartLinkArrangement 函数。目前没有命题与命题的情况对吧？
或者是例子与例子，反例与反例，或者是这三种与这三种其一的情况，都按照下面的处理：和定义与定义的类似。

希望的效果是，假设是卡片 A 和 卡片 B
1. 检测卡片 A 的最后一条评论是否是 B 的链接，以及 B 的最后一条是否是 A 的链接，两个都是的话进行下一步
2. 在 A 的“相关思考”字段的 bottom 下方增加一个“- ”，然后把 B 的链接移动到这个“- ”下方，同理处理 B 卡片。其实这个操作和定义与定义的是一样的。

并且把上面说的这几种情况的功能优化到 mntoolbar 里的 moveUpThoughtPointsToBottom 动作，目前这个单击动作只有一个功能：把自动获取的内容移动到“相关思考”，但希望能够把上面的处理进行兼容。ultrathink
---
manageCommentsByPopup 需要优化:现在是把移动和删除的确认放在了新的弹窗里，但是这样会有点麻烦，我希望的是如果第一个弹窗里选择了“多选”，那么就把这个选项直接放在了多选的那个选择弹窗的下方（目前只有一个确认）。
因为弹窗多的话，确认起来就比较繁琐，虽然现在功能很丰富。ultrathink
---
今日看板那个功能继续开发：现在看不到任务类型，只能看到状态，这个不太好，因为我们启动任务肯定是做“动作”类型的任务。另外我是学生，很多时间都在读书，所以重点研究一下如何拆解读书任务？目前有的那些拆解是很久之前写的，并不完善！希望基于目前的框架和体系重新写。
注意一定要灵活，能方便地调整和追踪。 ultrathink
---
优化一下制卡功能：
目标、关键结果卡片比较特殊，他们在“同一条”下是唯一的，也就是一个目标的子卡片只能是关键结果类型的卡片，关键结果类型的卡片的子卡片只能是项目类型，所以这两种情况就不需要弹窗选择了，就按照上面说的制卡就行，其余的情况不确定，所以才需要手动选择。ultrathink
---
继续开发 mntask：
目前增加一个今日任务的识别符号，然后筛选的效果是把他们都移动到今日看板。但这样有个问题是还要考虑到后续把他们移动回去的问题。

所以我觉得就是在今日看板里通过链接或者 Markdown 形式的行内链接比较好，这样那个“今日看板”可以更加宏观地区看到任务情况，所以其实就是把今日任务的一些处理，都最后呈现为 “今日看板” 卡片里的评论的处理。

所以请你优化一下。ultrathink
---
MNMath 的 smartLinkArrangement 函数有点问题
1. 命题卡片A这种知识点卡片和归类卡片B双向链接之后触发的话，A 中的B 链接，应该移动到“相关链接”的 bottom，而不是 top，top 有个问题是会被后续制卡的时候 linkParent 给清除掉
2. 加一个新的支持：定义 A和归类卡片B 的情形：A 在 B 内的链接移动到“所属”字段的 bottom
---
有一个需求：我希望能够把一些卡片“pin 住”，相当于收藏一些卡片，并且能够随时调用，在 FloatMindMap 里显示。
具体的交互设想如下：
1. 如何收藏卡片
   - 我想的是在 toolbar 的视图里增加一个新的 view（你可以重点学习一下 toolbar 里的切换视图(搜索 tabView)功能。）然后里面提供两个输入框，第一个写标题，也就是最后调用的时候显示的内容，第二个就写卡片的 URL 或者 ID，mnutils/xdyyutils.js 里面有一些 String 的方法可以判定和处理，比如统一处理为 ID 之类的，但是其实问题都不大，因为正确的 URL 和 ID 都能作为 MNNote.new 的参数，然后 focusInFloatMindMap 就行了。
   - 但上面只是我暂时想的，重点其实是如何设计的可以方便加入，修改，删除和调顺序
     - 修改的话，就是点击之后两个输入框里就显示对应的信息，修改后保存就可以更新，有点类似于第一个按钮视图的设置那种感觉
     - 还希望能支持：如果ID 那个输入框是完全空的也能加入，因为这个时候会作为一种分隔符来分隔卡片（万一太多的话）
   - 我突然想到可以类似于第一个按钮视图那种，他也支持改顺序，每个按钮下方也是会有两个框！我发现可以仿照那个视图弄一个！
2. 如何调用卡片
   - 我其实之前弄过一个按钮，应该是带有 `pin` 的一个，然后 toolbar 支持替换软件原生的菜单，这样的话，我就用 pin 来替换原生的菜单，当然也可以不替换，但反正通过`pin`按钮，我单击一下就可以看到一个 pin 菜单，每个 item 都可能是卡片（也可能有一些 item 是用来划分的），然后点击具体的标题就可以在浮窗中定位到对应的卡片
   - 或者你想想看怎么弄比较好？

关键点
1. 这个信息需要能够被保存，所以你需要看看如何把这些信息存到 iCloud 里，这个在 toolbar 里是可以做到的。
2. 一旦你需要在 xdyy*.js 外的文件里进行修改，要注意这些文件是官方的文件，更新的时候需要用 update.py 来打补丁，免去我手动修改的麻烦。
ultrathink
---
请学习 mnai, mntoolbar, mnutils 项目，前两个学习卡片的更多处理的可能性以及一些结合插件实现的操作， mnutils 学习基础 API。请你进一步规划思考任务管理需要的筛选，并且完善今日任务的筛选以及后续处理，并增加其它类型的筛选 ultrathink
---
请你阅读 mntask 的 TaskFieldUtils 和 MNTaskManager 两个类，了解目前已经开发的一些功能。基础 API 参见 mnutils 项目
现在打算开始开发 mntask 的筛选功能了，因为目前只切换状态的话，会有很多进行中的任务
我们首先解决:
1. 要有一个今日看板？（在 Task Board 视图里添加绑定）
2. 如何标记哪些是今天要做的？比如在信息字段下加什么子字段？
3. 如何把这些卡片筛选出来？
4. 筛选出来之后如何确定这些任务的顺序、优先级？也就是先做哪件再做哪件？
5. 做完了带有今日任务标记的任务，今日看板内如何更新？
ultrathink
---
mntoolbar 里的 moveNewContentsByPopupTo 动作和 deleteCommentsByPopup 动作，本质上是 MNMath 里的两个函数。我希望他们能统一。

现在moveNewContentsByPopupTo 对应的 moveCommentsByPopup 函数对评论的多选处理和解析已经很完善了，后续就在这个上面进行进一步开发。我希望能把删除评论的功能加进来。我的大致思路如下：
1. 第一个弹窗整体，依旧是选择获取评论的方式
2. 第一个弹窗的几个选项触发的第二个弹窗里都加一个按钮，功能就是“删除”，然后原来的“确定移动”类型的功能，改成“移动”，具体的用词你可以调整，反正就是这个窗口就可以选择下一步行动是删除还是移动，如果是删除，就再给一个弹窗，弹窗的提示内容里把我选中的内容的类型和内容都列出来（就和弹窗里解析显示的是一样的），点击确认的话删除就可以了。
如果选择移动，则按照之前的处理即可

开发完之后应该更新一下 mntoolbar 那个菜单里的动作，应该是可以把删除评论的去掉，以及删除评论的函数也可以去掉了。
ultrathink
---
我发现移动和删除评论的弹窗函数里都有一个问题，就是归类卡片的前缀有问题
```
“xxx”相关yy
```
的归类卡片的类型应该写成 `[yy归类]` 而不是 `[yy]` ultrathink
---
刚测试了还是有个 bug: 命题卡片证明字段的内容,如果移动到"相关思考"字段下方的某个评论 A,我刚测试了点击"A上方"和"A 下方",结果都是到了 A 上方 ultrathink         
---
第二层解决了，但是第三层没有完全解决，虽然我也能接受，但是你看这个图 [Image #1], 明显是 HtmlMarkdown评论字段下方的内容暴露出来了! 比如那个"倒数第一"到"纯手写" 这个部分应该隐藏在 "的回答很好的"这一项里面 ultrathink
---
再补充一下，刚刚的例子中，第三层也是和第二层同理，要有一个第三层的粗略位置，也就是 1 前面，1 后面， A，A top，A bottom，B，B top，B bottom，点击 A 或者 B 进入更细节的位置控制 ultrathink
---
你说的第三层,我怕你没理解正确,我补充一下,这个时候暴露出来的普通评论的前面应该是没有任何 HtmlMarkdown 评论的,一旦有,就和前面说的一样,一定属于某个 HtmlMarkdown 评论,那么这个时候就不能暴露在这层,比如 1, A, 2,B 3. 其中  A 和 B 是 HtmlMarkdown 评论，然后从左到右 index 增加，这个时候 1 就是需要和 HtmlMarkdown 同层，因为它不属于任何 HtmlMarkdown 评论，但是 2 属于 A， 3 输入 B，所以 2 需要点击 A 才能看到，3 需要点击 B 才能看到。 ultrathink
---
还是有问题：这张图
第二个弹窗怎么又把字段里的内容暴露了？
这个部分
这个时候不应该显示的，这个部分的内容，应该点击“证明区”然后在第三个弹窗里显示
第三个弹窗也是这个类似的问题：
应该像第二个弹窗一样，先提供每个 HtmlMarkdown 评论的 top 和 bottom，然后如果有进一步需要，再点 HtmlMarkdown，比如我点击 “你干嘛啊”，才到第四个弹窗的内容。

总结一下，其实现在要的效果有了，但是第二个和第三个窗口都有暴露内容的问题。还没把这个“文件夹”分得很彻底 ultrathink
---
继续开发 mntask: 现在任务只有三个状态，但是其实第三个状态可以点击后进行归档，我希望在状态栏那个地方也增加一个“已归档”状态，来与“已完成”区分，因为“已完成”可能会退回“进行中”。 ultrathink
---
我刚发现应该还需要一个弹窗，因为目前第三个弹窗是 HtmlMarkdown 评论与其下方的所有内容都暴露在一起，容易混淆。
应该是类似于第二个弹窗，如果要更精细移动化，再点 HtmlMarkdown 评论，这样就进入到第三层弹窗，里面的内容是 HtmlMarkdown 下方的所有内容。 这样才真正能实现三层，像文件夹一样的效果。ultrathink
---
这张图左边是具体的卡片，右边是第二层弹窗，有个问题是这个时候 HtmlMarkdown 评论的字段不应该显示的，不应该在这一层就暴露出来，只有当我需要进一步点击证明区的时候才需要，但我看点击后第三层的功能确实也实现了，请把这个第二层优化一下  ultrathink
---
1. 现在第一个弹窗里根本无法像 deleteCommentsByFieldPopup 那样多选！ ultrathink
---
1. 如果是解析的是纯链接，那么弹窗里显示的应该是标题，不过如果有前缀的话需要处理，参考一下别的函数的处理，其实就是处理为`[类型] 标题`
2. 手写内容没有正确识别类型，可以学习一下 deleteCommentsByFieldPopup 函数的类型处理。要注意用 note.MNComments 来获取更精细的类型
3. 你目前也只是开发了两层，你相当于把 HtmlMarkdown 评论和其它评论看成一种了。在第二层的时候，其余类型的评论不应该“暴露出来”，应该要进一步点击某条 HtmlMarkdown 评论才行。所以你得学习 MNMath 的字段识别和字段下方内容获取，HtmlMarkdown 评论也要有一套类似的。ultrathink
---
继续开发 mnutils

## moveCommentsByPopup 增加新功能
目前我们 xdyyutils.js 里的 MNMath 类有一些能够解析普通评论的函数（不是函数本身，而是内部实现了），你可以学习一下。这里普通评论是指文本手写，图片，链接之类的，也就是说，不是我们 MNMath 里专门的字段或者是 HtmlMarkdownUtils 类里加的这种评论，虽然它们本质上也是 markdown 评论。

目前的 moveCommentsByPopup 函数的功能是只实现了两层。第一层是 HtmlComment 类型评论的字段，第二层是点击 xx 区“进入了第一层到达第二层”（这里的“进入了第一层”是指获取了这个字段下面的内容，也就是到下一个字段之间的内容。）

然后我们目前可以解析 HtmlMarkdown 评论。但现在有一些还没考虑到的场景：
1. 第二层里没有 HtmlComment 类型评论的字段，而是普通评论
   - 这种情况下就需要能够解析普通评论，从而能够移动内容到这种评论的位置，也就是能精准定位。
2. 如果有 HtmlComment 类型，先解析，然后在现在的基础上进一步处理：
   - 如果两个 HtmlComment 类型 A 和 HtmlComment 评论 B 之间有普通评论，那么 A 的两条（也就是 A 上方和 A 下方）就增加一条，点击后可以“进入 A”，也就是到了第三层，再有一个弹窗，里面的内容是解析的 A 和 B 之间的评论，然后这个时候就只需要有某条“下方”，以及最开始和最下方即可，因为这样已经可以取遍所有index 了
   - 如果 A 和 B 之间没有普通评论，那么就保持只有 A 的两条，也就是 A 上方和 A 下方


## moveCommentsByPopup 触发后的默认效果需要优化
- 不需要关键词（这个有其它函数处理过，你可以学习一下，关键是是不显示“关键词”，但是不要影响“关键词”上面一个和下面一个字段的处理
- 加一个“最后一条评论”按钮，和手动输入“Z”是一个效果，因为我发现我经常移动最后一条评论。
ultrathink

## 选择移动的内容也要继续优化
在选择移动什么内容（也就是第一个弹窗的选项）时，目前能选择的内容只有
- 自动获取
- 手动输入 Index
- 某个具体的字段下方的全部内容

无法更加精细化地选择。目前已经在其它函数里通过弹窗实现了多选。

希望也能像刚刚的提到的那样，进行三层的解析，如果进入到第三层，或者没有 HtmlComment 评论时的第二层，可以通过弹窗选择，也可以多选，多选其实就是涵盖了整个字段的处理。